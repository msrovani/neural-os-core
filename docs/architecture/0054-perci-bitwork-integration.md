# ADR-0054: Integração Perci Bitwork → .bitnet + Trinity MoE

**Status:** Proposed (pesquisa) — **adiada** (2026-07-17); sem wire no tree
**Ex-ID:** ADR-0045 (conflito com Sound); migrada para 0054 para liberar ID canônico Sound
**Data:** 2026-07-15 (análise); renumerada 2026-07-17
...
> **Postergada:** não entra na fila ativa (Sprint Net / residual Sound / ADRs 0051–0053). Retomar só com OK do maintainer após Net RX estável.
...
| **Tamanho** | 49KB–270KB (experts) / 202MB (2B) | 200MB (peso) |
...
**Recomendação (quando desbloquear):** implementar o `CognitiveRouter` como novo tipo de modelo no loader `.bitnet` (extensão de header), integrar no topo do `classify_intent()` do Trinity MoE. O Bitwork vira o **router treinado definitivo** — substitui tanto o keyword matching quanto o router ternário subótimo. A geração continua no BitNet 2B (ou expert específico).
