...
| `0105-gpu-multi-isa-kernelimage-falcon3-w2a8.md` | Accepted (MVP) | `fazendo` | **#550**, #536 | **KernelImage + AIOS dual-GPU + Falcon3 1B/3B/7B/10B W2A8.** Loaders CUBIN/HSACO/zebin; Ready=pack+golden; device W2A8 Layer S. SESSION_361 |
| `0106-decisoes-calibradas-confianca-abstencao-hitl.md` | Proposed | `por_fazer` | **#588** | **Decisões calibradas.** Contrato `Decision<T>` (opções fechadas + distribuição Q8 + `confidence`/`margin` + abstenção `Unknown`); θ derivado de reliableza medida (R2 sanciona / R3 aplica / bin pina; `Deny` imune a confiança); sítio conversacional abster→LLM vs ação abster→HITL; labels = HITL + resultado verificado (fim da destilação da heurística); **onda M** migra os sítios de decisão por substring (~497 ramos, proxy) com paridade por sítio — parsing **não** migra. Não substitui 0088/0083/0104 |
