...
| `0060-bitnet-cognitivo-bei.md` | Proposed | `por_fazer` | #470–#478 | **BitNet Cognitivo (BEI):** 7 ondas — MPMC, economia, células, MoE dinâmico, memória L0-L7, afeto contínuo, supervisor meta-cognitivo, Soul Mirror. Absorve #218/#224/#314/#315.13/#315.17/#168/#190/#199/#468. Depreca #136. Ver `.opencode/plans/0060-bitnet-cognitivo-bei.md` |
| `0062-claudioos-vs-neural-aios.md` | Proposed | `pesquisa` | #479–#485 | **ClaudioOS vs Neural-OS:** estudo comparativo profundo (código lido); adoção seletiva de TLS 1.3, VFS, AHCI/NVMe, Limine, IPC, async híbrido. Não adota thin-client/single-address-space/Win32 |
| `0063-tickv-noproto-ai-index-sgdb.md` | Proposed | `por_fazer` | #491–#505 | **TicKV + NoProto + Índices IA como SGDB:** Storage Engine TicKV (Append-Only NVMe) + Schema NoProto (Zero-Copy) + ART Index (L0-L3) + BQ Flat SIMD (L4-L5). Elimina SQL/FS tradicional. Power-loss safety, zero-copy, SIMD dispatch unificado. Supersede NeuralFS/ADR-0040 para persistência IA. |
| `0064-rag-db-in-kernel.md` | Proposed | `por_fazer` | #486–#487 | **RAG DB in-kernel:** VectorStore TF-IDF + cosine (ref. ClaudioOS `vectordb.rs`); crate `vector-db` em cortex; L1 da memória cognitiva (ADR-0060 Onda 4); embeddings neurais = F6 residual gated VRAM |
...
- **ADR-0040 NeuralFS (persistência IA)** → **ADR-0062** (TicKV + NoProto + Índices IA como SGDB primário).
