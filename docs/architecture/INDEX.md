...
| `0047-gpu-compute-pipeline.md` | Accepted (MVP parcial) | `completa` | Extensão GPU | G1+G2 PoC; G3–G5 + DP4A defer. **§7 SASOS reconciliado com ADR-0087 §2.0.1 (2026-08-06):** dono do Tier 0 = 0087 (SASOS 4a + CE 4b); este ADR define layout SASOS + consumo `Tensor::location = MemTier::Vram` |
...
| `0055-smp-revision.md` | Accepted | `fazendo` | #16–42 + **#492** | **Canônica SMP.** SESSION_279 trampoline+MADT u32; SESSION_281 ICR x2APIC + GDT 1 TSS/CPU. Aceite metal K23/`online==madt-1` aberto. TCG `max_aps=4` = gate ambiente. |
...
| `0057-compute-dispatch-smp-gpu-npu.md` | Accepted | `completa` | #20–42, #211, #329–331, #345–346, #454–456 | WS-A–E/G ✅ código. WS-F: IDT+TSS **por CPU** (SESSION_281 `k_nano::gdt`); `ap_pollable`/matmul HW + IPI reschedule = residual metal. |
| `0058-generative-card-desktop.md` | Accepted | `completa` | #79/#80/#82/#279d/#283/#448/#452/#465 + #468 + **#541** | **UI/Desktop Jarbas:** embedded-graphics `DrawTarget` + `UiDeclaration`/`UiRenderer` (cards). **S1–S4 ✅**. **s294** compositor hot path (fill scanline + glow isqrt). S5 + A/V real residual |
...
| `0101-falcon3-3b-cognitive-lab.md` | Accepted | `fazendo` | **#544** | **Lab 3B-first.** Falcon3-3B Instruct 1.58-bit. s387: `set_model_header` wired Observe. Residual: metal tok/s, #549 tokenizer/KV. |
...
| `0104-cadencia-tick-adaptativa-timercap.md` | Proposed | `fazendo` | **#556** | **Cadência de tick adaptativa (TimerCap).** Alvo deixa de ser const; R0 mede (LAPIC/TSC/jitter/alive) + `set_tick_hz`; R1 `k_hal::timer_cap` sanciona faixa + `request_tick_hz` (rails {30,60,120}/dwell 5s/confirmação/rollback, auto só desce, subir>default=HITL); R3 jarbas objetivo de frame; bin dirige + `/tick`/`CONFIG.TXT TICK_HZ=`. Motivada por `TIMER_TICKS=0` (x2APIC) + ~1 Hz no metal. Não substitui 0088/0041/0092 |
| `0105-gpu-multi-isa-kernelimage-falcon3-w2a8.md` | Accepted | `fazendo` | **#550**, #536; F4→0084 | **Canônico W2A8.** B0–B3 código ✅ (FAT/aliases, CpuOnly honesty, CPU ladder+gaps, telemetria). B1.2/B2.4/B4 = AWAITING_HW. **s410c**: lab KVM+VFIO (`tools/run-qemu-kvm-vfio.ps1`) com canários preservados |
| `runtime-hygiene-checklist.md` | Implemented (checklist vivo) | `feito` | **#618**; lição s410 | **Runtime hygiene: cap + evicção para toda estrutura de aprendizado.** 5 perguntas (cap/evicção/dedup/dreno/por-unidade) + tabela auditada s410d: 7 estruturas corrigidas (EMBED_INDEX, trust escalation_log, fts IDX, TASK_PATTERNS, EVOLVING, SKILL_PROVENANCE, JOB_RINGS) + ~19 pré-existentes com cap. Processo p/ agente novo: heap estável em janela longa. |
| `0106-decisoes-calibradas-confianca-abstencao-hitl.md` | Implemented | `feito` | **#588** | **Decisões calibradas.** Contrato `Decision<T>` (Q8 + margin + abstain); θ da reliableza (`Deny` imune); conversacional→LLM / ação→HITL; labels HITL+weak_auto→JSONL→`train_router` (HITL×3); onda M: M0 triagem, M1 emotion/skill, M2 plugin/market, M3 Hub `decide`, M4 OUT (`0106-m4-excluded-sites.md`). **s364–s366.** Residual opcional: r3 replay contínuo + ECE. Não substitui 0088/0083/0104. |
| `0110-laya-zero-implementation-adr.md` | PROPOSED (revisada) | `por_fazer` | **#611**; ex-#588 gloss | **ADR-0110 rev. 2026-09-25 (SESSION_407)**: revisão derrubou ACCEPTED ilegítimo. REJEITADO: RAM 150-300MB pinned; <35ms garantido; **reuso hw_expert_v6 como classificador de intenção** (erro de categoria: input=vid/did→família device; S248 transformer errado, teto 59-63%, gate #492 ≥65%); **TickV TTL efêmero** (TickvLite é persistente; efêmero=ring RAM S352); skip-detectores/consenso-drop (contraditórios; SecurityAgent não dropa). ACEITO: TaskType::LayaIntent=8; triagem aditiva fail-closed com `abstain(ScorerAbsent)` (0106). **Adendo §10**: Laya real verificado (Convai ModernBERT 421M, Apache-2.0, 842MB fp16, 193-464ms CPU, zero-shot≈acaso, ECE 0,466) → rodar no metal REJEITADO; rota #611 = **destilar contrato sobre `decide_intent`/BitNet trained**; `Decision<T>`≡primitivas boolean/choice/score do Laya; agent-jev=AgentJev-0.6B (Qwen3 decoder→classifier, mesma categoria). Citações corrigidas: 293→273, 280→242, 152→310; +248. **TaskType::LayaIntent=8 implementado (protocolo apenas, SESSION_408)**; scorer/triagem runtime seguem bloqueados por #611. |
