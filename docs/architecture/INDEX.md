...
| `0055-smp-revision.md` | Accepted | `fazendo` | #16–42 + **#492** | **Canônica SMP** — HwReal=MADT−1. SESSION_279: trampoline+IDs u32 (anti-teto 8). TCG `max_aps=4` ainda gate ambiente. SESSION_141 histórico TCG/WHPX. |
...
| `0087-mhi-real-dma-multi-tier.md` | Accepted | `fazendo` | #420, #423, ADR-0040/0046/0048-50/0062 | **MHI Real — DMA Multi-Tier (2026-08-05):** preenche o gap #420/#423 — migração real entre tiers via engines. Pesquisa profunda: NVIDIA CE Pascal→Blackwell (0xc1b5→0xcab5, methods 0x0260/0x0400/0x0300, privileged, fence USERD→semaphore), AMD SDMA (AWAITING RDNA), Intel BCS (fix 0x22000 suspeito), NVMe PRP zero-copy (mata bounce de 1 página — QEMU testável), AHCI PRDT direto, política estilo Linux memory-tiers (tier ids, histerese, promoção async). Veredictos: P2P/GDS ❌ (hairpin), IOMMU ❌ (overkill). Design: `record_access` wire (hoje sem callers) → policy → mhi_tick dispatch por engine. Roadmap F1-F6 (NVMe PRP → MHI wiring → BCS → **4a SASOS VRAM no heap [0047-GPU §7.4] → 4b CE GTX 1050** — reconciliação 0047-GPU: SASOS (ponteiro, acesso pontual) + CE (engine, bulk) complementares; dono do Tier 0 = esta ADR; WC p/ gravação VRAM via CPU, UC p/ leitura). |
| `0088-aios-first-premissa-maxima.md` | Accepted | `fazendo` | **#512** #513 #534 #535 #537 | **PREMISSA MÁXIMA AIOS-First.** SESSION_271–274: DeviceTree H1 + plano k_ai (NIC+storage+Trust+HITL); Trinity único + MoE/HUD honesto; GPU matmul sem fingir device + MHI tier0 CE wired. Residual #513: measure_bandwidth. |
| `0089-novo-hermes-malha-cognitiva-global.md` | Proposed | `pesquisa` | Whitepaper Novo Hermes | **Malha Cognitiva / Novo Hermes (2026-08-09, v4):** whitepaper de engenharia. **v4 = auditoria de viabilidade** (alucinações/devaneios removidos): mesh real = 16 nós, node_id u8, só UDP L2 42069, teto payload ~64KB (FRAG 64×1000); JÁ trafega P2P: skills (`SKILL\0/PROMOTE\0`), marketplace, `CRDT\0`, FL `FD\0/FM\0`+`FED\0`, experts `EDR\0`, memória `MEM\0/SOUL\0`, matmul `MW\0`. **Removido como inviável:** ZK-STARKs, atestação TPM (tpm.rs só tem extend_pcr, sem Quote), tensor sharding 70B, eBPF (kernel não é Linux), Wi-Fi Direct/BLE no bare-metal, PeerID libp2p (sistema é u8), WASI-NN/llama.cpp (não existem). **ARQ = ART + BQ + Recall + Doc** (definição do maintainer; "Índices IA" da ADR-0063). NoProto/TickV = repos upstream usados como base, reimplementados in-tree (ADR-0063:185-201). **Foco v4:** o que o kernel JÁ faz + **apps host** (Rust std, fora do kernel) que falam o mesmo protocolo NoProto/42069 e fazem WAN/IoT (kernel só cliente; sem listener TCP — `log_agent.rs:4`). Proposta: crate `mesh_proto` compartilhada + Runner Host app. Contratos novos com node_id u8: `TaskType::{Replicate, Revoke}` + `BL/` CRDT + atestação de binário Ed25519 (sem TPM). Modo Sonho = evolução do `SleepCycleAgent` real (quórum + antígenos, sem treino). C-Watt = extensão de `k_ai::economy` sem ZK (fuel+hash+assinatura). Estende ADR-0081 (malha) + 0063 (SGDB) + 0059 (wasmi) + 0088 (Premissa Máxima). Pós-v2.0; fases A–E. |
