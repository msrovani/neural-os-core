...
| `0059-runtime-app-factory.md` | Accepted | `fazendo` | #103/#309a/#385–396/#402/#411/#8/#11/#306 + #469 | **App feita por IA em runtime:** **A (wasmi) ✅** (`.wasm` real PASS) + seletor A/B/C ✅ + **F7 arena W^X ✅** (nativo `mov eax,42`→42). F6 movido → **ADR-0060**. Supersede ADR-0031(WASM)/0032 |
| `0060-ring3-isolation-ring.md` | Proposed | `pesquisa` | ADR-0059 F6; #426 | **Ring3 Isolation Ring (F6):** destrava B/C nativo. **BLOQUEADOR** (habilitar=triple-fault reboot loop). Porto seguro documentado (kernel OK, ring gated). Conectores no código (`isolation_ring` + `app_factory::register_native_ring`). Critérios de aceite §6. Sessão debug dedicada |
