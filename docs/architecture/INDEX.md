...
| `0110-laya-zero-implementation-adr.md` | PROPOSED | `por_fazer` | **Análise Laya-Zero** | **ADR-0110**: Avaliação viabilidade white paper Laya-Zero. Decisões: (1) Bloquear 150-300MB RAM no boot REJECTED; (2) Garantir <35ms forward pass REJECTED como garantia absoluta; (3) Fase 1 ACEITA: classificador Laya no k-ai reutilizando hw_expert_v6; (4) NoProto+TickV para dados efêmeros ACEITO; (5) Triagem segurança no kernel ACEITA. Lifecycle: `por_fazer` → `fazendo` após dispatch especialistas @explorer/@fixer/@librarian. Impacto: ~60% valor conceitual com ~20% esforço arquitetural. |
