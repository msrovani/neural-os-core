...
| `0062-tickv-noproto-ai-index-sgdb.md` | Proposed | `por_fazer` | #491–#505 | **TicKV + NoProto + Índices IA como SGDB:** Storage Engine TicKV (Append-Only NVMe) + Schema NoProto (Zero-Copy) + ART Index (L0-L3) + BQ Flat SIMD (L4-L5). Elimina SQL/FS tradicional. Power-loss safety, zero-copy, SIMD dispatch unificado. Supersede NeuralFS/ADR-0040 para persistência IA. |
| `0063-rag-db-in-kernel.md` | Proposed | `por_fazer` | #486–#487 | **RAG DB in-kernel:** VectorStore TF-IDF + cosine (ref. ClaudioOS `vectordb.rs`); crate `vector-db` em cortex; L1 da memória cognitiva (ADR-0060 Onda 4); embeddings neurais = F6 residual gated VRAM |
