...
| ~~I2~~ | ~~FAT32 format não implementado~~ → **corrigido**: `format_fat32_esp` existe | ADR-0079-plan 1.4 | ✅ já coberto por `fat32.rs:1043` |
| ~~I3~~ | ~~**Limine no target**: SysInstaller copia a ESP crua por setor (sem escrever limine.conf específico do alvo)~~ | ✅ **resolvido 2026-08-05** | ESP copiada crua **é** o bootloader instalado (contém Limine + kernel.elf + UPDATE.CFG); AutoInstallerAgent executa `install()` de verdade (SYS_INSTALL → run_install_from_bus → AHCI/NVMe/USB) |
...
| ~~I5~~ | ~~**Leitura NeuralFS de modelos** no boot (hoje só FAT32/exFAT)~~ | ✅ **resolvido 2026-08-05** | `try_hub_slot_neuralfs` (main.rs): monta NeuralFS 0x7F, lê `/models/`+`/boot/`+raiz antes do FAT32; ModelProvisioner persiste em `/models/` — Residente carrega sem re-baixar |
...
| ~~I7~~ | ~~**`hw_profiler::gpu_vram_mb = 2048` hardcoded** p/ NVIDIA — decisão "GPU 4GB+ VRAM → 7B" depende de VRAM real~~ | ✅ **resolvido 2026-08-05** | `bar_size_mb()`: técnica PCI (0xFFFFFFFF → bits de tamanho → restaurar) no BAR0 da GPU — VRAM real, sem hardcode; decisão 2B/7B usa VRAM verdadeira |
| ~~I8~~ | ~~**`self_check::verify_install_checksum` é placeholder** (sem walk de diretório)~~ | ✅ **resolvido 2026-08-05** | `resolve_path("boot/kernel.elf")` + `read_file` + CRC32C vs INSTALL.CHK — verificação real pós-instalação |
...
| ~~I12~~ | ~~**Imagem instalável fixa não é default** — `MODELS_SOURCE=network` é opt-in (ADR-0079 §2.8); builds atuais ainda embutem modelos na imagem~~ | ✅ **resolvido 2026-08-05** | `build_image.py --mini`: `PACK_LLM=none` (só HwExpert+experts+firmware) + `MODELS_SOURCE=network` no CONFIG.TXT — imagem ~60MB, alvo baixa o brain no 1º boot (testado: raiz sem modelos grandes) |
