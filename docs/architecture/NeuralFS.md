...
- **F4b — Árvore de checksums por bloco (FEITO, v1.9.9 TEST)**: `ItemType::Checksum` (0x05) agora é usado — `write_file` grava um item `(ino, Checksum, bloco)` com o CRC32C da página de 4096B COMO GRAVADA (padding incluso, determinístico) na mesma b-tree (inode_tree_root); `read_range` verifica página a página via `btree_lookup` ANTES de copiar e recusa `block crc mismatch` — bit-flip detectado no streaming (AirLLM) sem reler o arquivo. Volumes legados (item ausente) ou crc==0 pulam (mesma política do F4). `checksum_tree_root` continua 0 — não é root separado (items convivem com inodes/extents, ordenados por (object_id, item_type, offset)).
