...
### WS-B — Paralelismo na inferência (✅ wired)
...
- `Tensor::matmul` roteado a `parallel_matmul` quando APs vivos.
- Aceite de **speedup = HW real** (TCG desliga AVX2 e emula; QEMU valida corretude/roteamento).
...
### WS-F/WS-G — futuros (não neste ADR)
- WS-F: IPI reschedule real, run-queues por core, per-CPU slab (#42), CFS opcional (#335).
- WS-G: Medusa no forward (#140), FlashAttention (#414), PagedAttention (#413),
  structured decode (#412), huge pages (#92/#93), burn-flex CPU (#333), codebook VQ (#169/#170).
...
- [x] QEMU `-smp 4`: 3 APs online + CorePools distribuído.
- [x] Dispatcher wired; `[COMPUTE]`/`[NPU-HW]` honestos no boot; sem panics.
- [ ] **HW real:** speedup de matmul multicore (WS-B) e GPU `Ready` + W2A8 (WS-D).
