...
**Última atualização:** 2026-08-09 — ADR-0089 Novo Hermes v4 (malha real + apps host) + ideias #514–#521.
...
| 2026-08-09 | **522** | **neural-sgdb — SGDB como projeto comunitário standalone (extração Modo 1)** — núcleo `k_ai::sgdb` extraído para repo separado `github.com/msrovani/neural-sgdb`, MIT OR Apache-2.0, zero deps, dual-mode `no_std`+`std`. Topologia decidida (HITL): evolução independente; OS mantém `k_ai::sgdb` AGPL interno, SEM fiação (porta futura = versão crates.io); interop por formato NMD1 byte-idêntico. Entregue: v0.1 (art/memory_doc/bq/hamming + Storage trait/InMemory/FileStorage + facade Sgdb) + feature p2p (CrdtMemorySync/Transport/UdpTransport) + benchmarks + MCP server (remember/recall/rag_context via JSON-RPC stdio). 4 commits, 20+1/24+1 testes, no_std check limpo. Roadmap 5/6 — TKLV interop adiado (exige leitor TickvLite do OS no host p/ verificação honesta). | ✅ implementada | ADR-0063 | SESSION_256 | `C:\DEV\neural-sgdb` (repo separado), `docs/api.md` (contrato) |
