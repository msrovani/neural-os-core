...
| 2026-07-14 | **424** | **K²CHJ Capability Rings MVP C** — Dois AddressSpace + CR3 switch + SharedSpscRing + Cap bitflags + trap `int 0x90`. Demo boot non-fatal. PoC Ring0↔Ring0; Ring3 (`iretq`) pendente. | ✅ PoC | Sprint 107 | ~400 | ADR-0041 + `address_space.rs` + `ipc/` + `syscall.rs` |
...
| 2026-07-14 | **426** | **SFI WASM + Cap contract** — Sandbox WASM com fuel + Cap por import sensível (net/ipc/FB). Evolução de #309a/#405 alinhada a ADR-0041 P3–P5. | 🟡 P3+ | Sprint 107+ | — | `wasm_rt.rs` + Cap |
| 2026-07-14 | **427** | **JARBAS FB MMIO + double-buffer (P4)** — Cap MAP_FB/WRITE_FB, AS map em JARBAS_FB_VA, backbuffer+present+vsync stub. Path bootloader FB; VirtIO-GPU BAR e Ring3 = follow-up. | ✅ PoC | Sprint 107 | ~280 | `jarbas_fb.rs` + ADR-0041 |
| 2026-07-14 | **428** | **K-IA DMA pin + Cortex weight mmap (P5)** — Cap PIN_DMA/MAP_DMA/MAP_WEIGHTS; pin frames + AS K-IA; mmap pesos eager em CORTEX_WEIGHT_VA; VirtIO/GGUF/demand-paging = follow-up. Roadmap ADR-0041 P0–P5 PoC completo. | ✅ PoC | Sprint 107 | ~350 | `k_ia_dma.rs` + `cortex_mmap.rs` + ADR-0041 |
| 2026-07-14 | **429** | **Ring3 user-mode real (P6)** — GDT user + TSS.RSP0 + `iretq` enter + stub USER (marker/`int 0x90`) + Cap::ENTER_USER + return kernel. Demo non-fatal. Follow-up: demand-paging #PF, VirtIO vring, GGUF mmap. | ✅ PoC | Sprint 107 | ~250 | `user_mode.rs` + `interrupts.rs` + ADR-0041 |
| 2026-07-14 | **430** | **Demand-paging via #PF (P7)** — lazy registry + reserve NOT PRESENT + #PF cure (frames pré-alocados); Cap DEMAND_PAGE; mmap_weights_lazy; demo first-touch non-fatal. Follow-up: VirtIO vring, GGUF/FAT mmap. | ✅ PoC | Sprint 107 | ~280 | `demand_page.rs` + `cortex_mmap.rs` + ADR-0041 |
| 2026-07-14 | **431** | **VirtIO vring + DMA pin (P8)** — Virtqueue layout-compatible sobre frames pinnados; Cap VRING_SETUP; Desc.addr=pinned phys; NIC live untouched (path paralelo). Follow-up: GGUF/FAT mmap. | ✅ PoC | Sprint 107 | ~220 | `virtio_vring.rs` + `k_ia_dma.rs` + ADR-0041 |
| 2026-07-14 | **432** | **GGUF/FAT file-backed mmap (P9)** — pré-fill frames via FAT `read_file_range` + demand-page lazy; Cap MAP_FILE; magic GGUF/BitNet verify; fallback NFIL se arquivo ausente. Follow-up: streaming on-fault seguro / >4 páginas. | ✅ PoC | Sprint 107 | ~260 | `gguf_mmap.rs` + `demand_page.rs` + ADR-0041 |
