...
| 2026-08-28 | **540** | **GGUF i2_s (type=25) support no kernel loader** — Falcon3-7B-Instruct-GGUF usa i2_s quantization (ternário {-1,0,+1}). O `cortex::gguf` só suporta Q4_0–Q6_K (types 2–8). Adicionar type=25 com dequant i2_s→f32 para carregar modelos ternários GGUF diretamente, sem conversão safetensors→v6. Reduz path de adoção: GGUF já é distribuição padrão da comunidade. | ⏳ | - | SESSION_293 | crates/cortex/src/gguf.rs |
