...
| 315.21 | **Pocket TTS Integration** (TTS principal) — Port do `pocket-tts-xn` (XN Rust, @LaurentMazare) ou `sherpa-onnx` (bindings Rust) para no_std. Kyutai Labs, 5.4K★, MIT. 100M params, CPU-native, ~200ms latência, 6× real-time, voice cloning, 6 idiomas. Alternativa: Piper TTS (11.2K★, C++). Fallback: Kokoro-82M (ONNX→.bitnet). Pós B-01 | 🔴 Pós B-01 | Sprint Sound | ~400 | kyutai-labs/pocket-tts + pocket-tts-xn + sherpa-onnx |
