...
| **#550** Gates G1–G3 GPU Rust ecosystem (cuda-oxide/cutile/khal como produtores NKP1 offline) | plano `docs/implementation/2026-09-13-gpu-rust-ecosystem-plan.md` | ❌ **Fase 2 descartada p/ sm_61** (SESSION_344): G1 ❌ piso cuda-oxide = sm_80 + Linux-only; G2a ✅ cutile-ir offline puro Rust (probe OK); G2b 🟡 tileiras offline mas CTK 13.2+ host e alvo sm_80+; G3 ❌ khal cpu-only quebra (bug upstream `any_backend.rs:30`). **Destra com GPU Ampere+ (sm_80) no lab + host Linux** — não Turing+. NKP1 segue nvcc como único produtor. |
