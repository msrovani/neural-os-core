...
**Última atualização:** 2026-09-19 — s360: mesh WHPX + fail/warn honesty (#585–#587); s359 bin (#582–#584); s358 cortex (#579–#581).
...
| 2026-08-28 | **540** | **GGUF i2_s/TQ2_0 (type 25) support no kernel loader** — Falcon3-7B-Instruct-GGUF usa quantização ternária {-1,0,+1}. O `cortex::gguf` só suportava Q4_0–Q6_K. **✅ Implementado em SESSION_309:** `TQ2_0` (type 25, 24B→32 f32) + BF16 (type 2) + tensor/metadata type IDs corrigidos p/ spec padrão + `GgufBackedModel` auto-config + GGUF magic scan no boot (`0x46554747`). Descoberta: o GGUF `-1.58bit-q2b0` do Falcon3 **não é ternário no arquivo** (155 BF16 + 45 F16); TQ2_0 fica p/ GGUFs ternários genuínos. | ✅ | - | SESSION_293 → SESSION_309 | crates/cortex/src/gguf.rs |
...
| 2026-09-20 | **#591** | **Trilha A BOOT.LOG UEFI** - metal sem MSC → stick vazio; soft-reboot loop sem writer. ✅ `logwriter-efi` + seal + **`init_from_phys` wired** + PMM reserve + Limine 12.9; soft-reboot API removida; PHASE/flush honesty. Residual: aceite `E:\BOOT.LOG` operador; uefi 0.40. | ✅ código; ▶️ metal | ADR-0088 / SESSION_169 | SESSION_369 | `logwriter-efi`, `boot_ramlog`, `boot/build.rs`, `mk_esp_fat.py` |
