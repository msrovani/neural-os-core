...
| **#550** Gates G1–G3 GPU Rust ecosystem + **Multi-ISA NKP1 (SESSION_361)** | **ADR-0105** B0–B4 | 🟡 **parcial** | B0–B3 código ✅ (FAT CUBIN sm75–89 via `pack_nkp_lab.ps1`, CPU W2A8+gaps, telemetria). B1.2/B2.4/B4 golden device = AWAITING_HW. |
| **#544** Falcon3-3B Instruct 1.58-bit = lab arquitetural (Falcon3-BitNet Cognitive) | **ADR-0101** | 🟡 Onda 0 | SESSION_298: 3B≠7B (HF); metal=scalar ADD/SUB; SIMD skip-native = gap. 1B comparativo; 7B não é o lab. |
...
| **#588** Decisões calibradas (contrato `Decision<T>` + confiança/abstenção + θ da reliableza medida + fim da decisão por substring) | **ADR-0106** | ✅ D0–D4 + M0–M4 (s364–s366); residual opcional r3 replay contínuo | s364 contrato+intent+site_policy; s365 typed_sites; s366 JSONL→train HITL×3, Hub `decide`, M4 OUT. Fonte Heuristic até head neural + ECE. |
...
| 2026-08-29 | **542** | **Mouse USB tablet QEMU não move** — após Runtime, `[MOUSE] display_tick status=0x1c obf=0 ibf=0 aux=0 pos=640x400` congelado; serial para de crescer no kernel lento (s294) mas pos nunca muda mesmo após compositor. HID tablet (`-device usb-tablet`) não alimenta `MOUSE_ABS_*`. Residual separado do paint. | 🟡 agendado | - | SESSION_294 | `k_nano/src/{interrupts,xhci}`, `jarbas/src/display/agent.rs` |
...
| 2026-08-31 | **544** | **Falcon3-3B Instruct 1.58-bit como laboratório** — shift DeepSeek V4-Flash / 7B-lab → *small reasoning engine* no 3B. Onda 0 = packing v6 + kernel ADD/SUB/SKIP no `x86_64-unknown-none` (hoje metal=scalar; AVX2 host=FMA f32). Ondas 1–3 = vocab/spec, adaptive compute, Cognitive Runtime. Não treinar early-exit/KL nesta sprint. Falcon-Edge 3B ≠ Falcon3-3B (shapes HF). ATLAS 7.1 tok/s é referência externa. | 🟡 | ADR-0101 | SESSION_298 | `cortex/{bitnet_avx2,bitnet_sse,nn,model_fit,model_hub}.rs`, `tools/convert_falcon3_*.py` |
...
| 2026-09-16 | **#558** | **Playback paceado pelo clock do áudio, não pelo tick** - `mixer` puxa 1024 amostras/tick; `write_hda_playback` é LPIB-aware e **descarta** o excedente (`PLAY_SAMPLES_DROPPED`). A 60 Hz puxa 61 440/s contra 16 000/s do dispositivo → rajada > 341 ms (buffer DMA = 1024 frames × 16 entradas) perde até ~74% da fala; 120 Hz ~87%; <15,6 Hz vira starvation. **ADR-0104 tornou o tick adaptativo (30/60/120) → a taxa da UI muda a fidelidade do áudio.** Fix: puxar `min(free_frames, ...)` (expor de k_nano) ou `expected = Δt_us × VOICE_RATE`; starvation = silêncio, nunca rajada. Aceite: `/tick 30/60/120` com `PLAY_SAMPLES_DROPPED == 0` (audição = metal). | 🟡 | ADR-0104 | SESSION_352 | `crates/jarbas/src/audio/mixer.rs`, `crates/k_nano/src/audio/hda.rs` |
| 2026-09-16 | **#559** | **Duplexidade explícita — auto-barge-in por eco** - VAD `start` com `PLAYBACK_RING.available() > 0` chama `request_interrupt()`, que limpa a fala **e** `cancel_active()` na inferência; não existe AEC no repo. Em HW real com alto-falante o assistente se interrompe e destrói a própria resposta (o piso do VAD ainda adapta para o próprio eco). Fix: half-duplex por padrão (ignorar frames/VAD enquanto fala e +150 ms), barge-in opt-in em `settings` com gate de energia acima do piso de eco medido em runtime; full-duplex só com AEC; **não** cancelar inferência no caminho de eco. | 🟡 | - | SESSION_352 | `crates/jarbas/src/audio/voice.rs`, `crates/jarbas/src/audio/{settings,vad}.rs` |
| 2026-09-16 | **#560** | **`STT_UNCERTAIN` falado (fim do silêncio)** - o tópico é publicado (`stt.rs:769`) e **ninguém escuta**: incerteza de transcrição = silêncio para quem fala. Consumir no `JarbasAgent` → TTS curto ("Não entendi, pode repetir?") + linha no chat; `[vazio]`/`[stt-offline]` seguem só na janela. HITL honesto (premissa 0088) da mesma frente que removeu o decoder que inventava texto (s346). | 🟡 | - | SESSION_352 | `crates/jarbas/src/audio/stt.rs`, `crates/jarbas/src/audio/jarvis.rs` |
| 2026-09-16 | **#561** | **Teto de utterance + CTC incremental (sem `n_frames × vocab`)** - `pcm_buffer` cresce enquanto `listening` e só sai no VAD end (15 frames de silêncio) → ruído contínuo = `Vec` sem limite; `begin_job` copia e aloca `logits = n_frames × vocab × 4 B` (768 KB p/ 30 s, 3 MB p/ 60 s) = mesma classe de cliff de s339/s349/s351. Fix: `MAX_UTTERANCE_S ≈ 12` com endpoint forçado + `VOICE_STATE`/log; greedy CTC guarda só a **emissão anterior** (collapse) → a matriz desaparece. | 🟡 | - | SESSION_352 | `crates/jarbas/src/audio/{voice,stt}.rs` |
| 2026-09-16 | **#562** | **Áudio é stream, não evento (ring SPSC em vez de EventBus)** - `bus.publish()` clona o payload **por assinante** em `VecDeque` **ilimitado**: `AUDIO_FRAME` (640 B, ~50/s) com ≥2 assinantes = ≥100 alocações/s e vazamento se um consumidor parar de drenar (agente dormente/rebaixado). Fix: frames por ring SPSC de consumidor único (sessão de voz) + double-buffer estático (mata o `flat_map().collect()` por frame); barramento fica com controle (`VAD_TRANSITION`, `VOICE_STATE`, `WAKEWORD`). | 🟡 | - | SESSION_352 | `crates/event-bus/src/bus.rs`, `crates/jarbas/src/audio/{capture,voice,wakeword}.rs` |
...
| 2026-09-21 | **#597** | **Upstream soft-float bumps** - crates.io: smoltcp 0.14, spin 0.12, x86_64 0.15. Workspace pin 0.13/0.9/0.14 por soft-float + API. Não bump cego no bin — sprint dedicado de compat. | 🟡 | — | SESSION_376 | workspace Cargo.toml |
...
| 2026-09-21 | **#599** | **wasmi 2.x no hermes** - crates.io wasmi 2.0; pin **0.47.2** (0.47.0 yanked, s379). Bump 2.x = sprint dedicado (API + features soft-float). | 🟡 | ADR-0059 | SESSION_379 | `hermes/Cargo.toml` |
| 2026-09-21 | **#600** | **CapGate tokens DynSkill** - Legacy(1) ainda autoriza DynamicSkill (Hermes EventBus). Migrar p/ DYNSKILL_TOKEN=0xD1 + trust_allow sem quebrar mesh. | 🟡 | ADR-0052 | SESSION_377 | `dynskill.rs`, hermes agents |
| 2026-09-21 | **#601** | **skill-registry honesty** - WASM execute teatro; Trust Contain bypass; cache all Ok; list_skills blob; FanOut/index/task mortos; 0 testes. ✅ fail-closed wasm; check_or_cache; idempotent cache; SkillListEntry; unregister; trim; 10 tests. Residual #599/#600. | ✅ | ADR-0052/0059/0088 | SESSION_377 | `crates/skill-registry` + hermes |
| 2026-09-21 | **#602** | **spin vs ticket-lock policy** - workspace ainda usa spin 0.9 em crates; crates.io spin 0.12 / lock_api 0.4. Não trocar FIFO por unfair sem evidência. Documentar: TicketLock/IrqSafe = statics SMP; spin = legacy/test. | 🟡 | SESSION_316 | SESSION_378 | workspace Cargo.toml |
| 2026-09-21 | **#605** | **hermes honesty s379** - evolve rollback candidato; CapGate Allow+stubs Ok; TLS ready sem bridge; ApprovalGate deny invisível; matrix "Aprendi"; signed=true; session_load dup; wasmi yanked. ✅ hot_swap live/prev; trap unwired; TLS deny; resolution; draft HITL; signed real; clear HNSW; 0.47.2; 203 tests. Residual #604/#599. | ✅ | ADR-0059/0088/0092 | SESSION_379 | `crates/hermes` |
| 2026-09-21 | **#542** | **Mouse USB tablet QEMU** - aux=0 esperado; MOUSE_ABS via P24b. InputAgent skipava sandbox. ✅ deferred HID sandbox + DisplayAgent pós-UI + mouse_is_ready. Residual EnableSlot TCG. | ✅ | - | SESSION_382 | xhci, InputAgent, DisplayAgent |
| 2026-09-21 | **#606** | **jarbas honesty s381** - energy sintética; slog info TRACE; barge-in cancela Infer; pcm unbounded; VGPU Offer=success; heapless morto. ✅ energy real; ADR-0092; interrupt TTS-only; pcm 30s; GOP\|\|kick; eg 0.8.2; 100 tests. Residual #597/#602 ( #542 ✅ s382). | ✅ | ADR-0058/0088/0092 | SESSION_381 | `crates/jarbas` |
| 2026-09-21 | **#604** | **hermes host ABI wire** - aios_net/fs Cap+bridge/VFS → I/O; orphans DEAD deleted; AGENTS A-019/020/023 ORPHAN; WPA2/InferenceFs/Decode/Ata. ✅ s380 205 tests. Residual #599/#600. | ✅ | ADR-0059 | SESSION_380 | `wasmi_rt`, `permission_gate`, orphans |
| 2026-09-21 | **#603** | **ticket-lock honesty** - Guard auto-Sync com T:Send; 0 testes; codemap MCS falso; IrqSafe clone sem repr(C); sem try_lock. ✅ Sync T:Sync; try_lock CAS; IrqSafe wraps TicketLock; docs IRQ; 7 tests. Residual #602. | ✅ | ADR-0088 | SESSION_378 | `crates/ticket-lock`, `k_nano/sync/irq_lock` |
