...
| **#550** Gates G1–G3 GPU Rust ecosystem + **Multi-ISA NKP1 (SESSION_361)** | plano GPU + ADR-0105 | 🟡 **parcial** | SESSION_344: cuda-oxide/cutile piso sm_80. **SESSION_361:** destruave ≠ Ampere-only — `pack_nvidia --sm sm_61` + RustCuda/Nvptx CompilerId; KernelImage CUBIN/HSACO/zebin; AIOS dual iGPU/dGPU; Falcon3 W2A8 shapes 1B/3B/7B/10B. Device golden = AWAITING_HW. |
...
| **#588** Decisões calibradas (contrato `Decision<T>` + confiança/abstenção + θ da reliableza medida + fim da decisão por substring) | **ADR-0106** | 🟡 D0 (`por_fazer`) | Auditoria s362: `think()` 14 intents sem confiança/`Unknown` (`cortex.rs:4593`); `ApprovalGate::classify` sem caller (`approval.rs:93`); `trinity::route` já faz softmax e grava distribuição (`trinity.rs:305-322`); `r3::update_with_replay` já treina por reward; `train_router.py` destila a tabela de keywords. **Onda M:** ~497 ramos triados, migra só IN=significado (OUT=sintaxe é contrato e fica), com paridade por sítio. |
