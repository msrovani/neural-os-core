...
| **#470–#478** BEI BitNet Cognitivo | ADR-0060 | 🟡 planejado | 7 ondas: MPMC, economia, células, MoE dinâmico, memória L0-L7, afeto, supervisor, Soul Mirror. ~2470 LOC |
...
| 2026-07-21 | **469** | **Runtime App Factory — WASM real (wasmi) + geração validada por IA** — Substitui a VM `Op` custom por **wasmi** (no_std, fuel, Apache-2.0) como runtime real de skills/`agent-wasm`. Pipeline "app por IA em runtime": Hermes detecta gap → Cortex/Trinity/LLM geram **WAT/DSL** sob **decodificação restrita por gramática** (evolução do #412 p/ CFG/PDA) → **assembler WAT→wasm no_std** → **teste em sandbox wasmi** → promover assinado a PackageHub `agent-wasm` (ADR-0052) → registrar tickável → card (ADR-0058). Bare-metal sem `rustc` → IA emite WAT/DSL, **não** Rust compilado (residual host). Refs: wasmi, AAGT, GBNF/Outlines/XGrammar, arXiv SelfEvolve/ARISE/Tool-Making/MCP-SandboxScan. Supersede ADR-0031(WASM)/0032. | 🟢 F0-F7 completo; F4 PONYTAIL (upgrade qdo `wat` no_std); F6 MicroPython.wasm stub dev | ADR-0059 | pesquisa crates.io/github/arXiv + audit código | `hermes/{wasmi_rt,wasm,dynskill,decode_harness,micropython_wasm}.rs`, `skill-registry/dynskill.rs`, `skill_opt.rs`, `evolve.rs` |

#### Deprecações

| Item | Motivo | Substituído por |
|------|--------|----------------|
| **#136** LLM decide memory tier | LLM no hot path de alocação é anti-pattern. Política determinística é mais rápida e previsível | ADR-0060 A.6 BudgetManager + FitPolicy #468 |

| 2026-07-21 | **470** | **BitNet Cognitivo BEI (main ADR)** — ADR-0060. Transforma BitNet de motor estático em organismo cognitivo: 7 ondas (MPMC, economia, células, MoE dinâmico, memória L0-L7, afeto contínuo, supervisor meta-cognitivo, Soul Mirror). Absorve #218/#224/#314/#315.13/#315.17/#168/#190/#199/#468. Depreca #136. ~2470 LOC, 16 arquivos, 0 dependências externas. | 🟡 planejado | ADR-0060 | `.opencode/plans/0060-bitnet-cognitivo-bei.md` | ADR-0060 + 7 ondas |
| 2026-07-21 | **471** | **Cellular BitNet (A.2)** — Micro-atores cognitivos concorrentes em cortex::cellular. CognitiveCell com inbox/outbox MPMC, CellNetwork grafo direcionado, CellScheduler round-robin. 3 cell types: Reasoning, Memory, Perception, Motor. Base para execução concorrente do BitNet. | 🟡 planejado | ADR-0060 Onda 2 | `cortex/src/cellular.rs` | Inédito no codebase |
| 2026-07-21 | **472** | **Living/Sparse Evolution (A.4/A.5)** — Plasticidade contínua dirigida por entropia em cortex::evolution. Morfogênese neural: spawn/prune células baseado em entropia regional. Bridge com SleepCycle #314 (batch pruning/consolidação) + EWC #314c (proteção) + Synaptic Homeostasis #314d (~18% redução). | 🟡 planejado | ADR-0060 Onda 2 | `cortex/src/evolution.rs` | Absorve #314/#314c/#314d |
| 2026-07-21 | **473** | **Memória Hierárquica L0-L7 (A.1)** — 8 tiers em hermes::memory/: L0 Cache (1KB), L1 Working (64KB), L2 Short (1MB), L3 Episodic (16MB), L4 Semantic (BGE), L5 Procedural (WASM), L6 VFS, L7 Archive (FAT32). Facade MemoryStore com promote_on_read ARC. Baseia-se no 4-tier pipeline #218 + Atkinson-Shiffrin #224. | 🟡 planejado | ADR-0060 Onda 4 | `hermes/src/memory/*.rs` | Absorve #218/#224/#281d/#282h |
| 2026-07-21 | **474** | **Afeto Computacional (A.7)** — AffectVector contínuo 5D (incerteza/urgência/fadiga/curiosidade/coerência) em hermes::affect. AffectRegulator modula roteamento: urgência>0.7→rota direta, incerteza>0.6→prefere LLM. Classifier frontend: EmotionState (ADR-0036) 7 classes → vetor. Visual: Soul Mirror (ADR-0047-HMI §7). | 🟡 planejado | ADR-0060 Onda 5 | `hermes/src/affect.rs` + `cognitive_bridge.rs` | Absorve ADR-0036 + ADR-0047-HMI |
| 2026-07-21 | **475** | **Supervisor Meta-Cognitivo (A.8)** — ExecutiveSupervisor unificado em hermes::executive. Núcleo: Ego Layer #315.13 (confidence tracking) + Babel-Index #315.17 (contradiction/entropy) + PonderNet #168 (inference budget) + 7-phase loop #190 + IterationBudget #199. Executa antes do router Hermes. | 🟡 planejado | ADR-0060 Onda 6 | `hermes/src/executive.rs` | Absorve #315.13/#315.17/#168/#190/#199 |
| 2026-07-21 | **476** | **Economia/Compressão (A.6)** — CompressionTier unified enum (1bit→2bit→4bit→5bit→8bit→f16→f32) em k_ai::economy. BudgetManager com FitPolicy #468 + IterationBudget #199. Tiers baseados em implementações existentes: Q4_0/Q5_0/Q8_0 (ADR-0046), VQ #169, TernaryTensor nativo. | 🟡 planejado | ADR-0060 Onda 1 | `k_ai/src/economy.rs` | Absorve ADR-0046/#169/#468 |
| 2026-07-21 | **477** | **MoE Dinâmico (A.3)** — Extensão de cortex::moe com birth/merge/split de experts em runtime. Birth via BitNetTrainer (ADR-0033), merge via SkillOpt #411, hot-swap via Evolve WASM #445. Trinity (#311) mantido como MoE estático base. | 🟡 planejado | ADR-0060 Onda 3 | `cortex/src/moe.rs` (ext) | Absorve ADR-0033/#411/#445 |
| 2026-07-21 | **478** | **MPMC queue** — Ring buffer lock-free multi-producer multi-consumer em k_nano::sync::mpmc. Slots com estado atômico (EMPTY→STORING→READY→LOADING). Baseado no padrão do SPSC #319 existente. Necessário para comunicação entre células cognitivas (A.2). | 🟡 planejado | ADR-0060 Onda 0 | `k_nano/src/sync/mpmc.rs` | Extensão do #319 SPSC |
