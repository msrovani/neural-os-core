...
| 2026-07-21 | **470** | **BitNet Cognitivo BEI (main ADR)** — ADR-0060. Transforma BitNet de motor estático em organismo cognitivo: 7 ondas (MPMC, economia, células, MoE dinâmico, memória L0-L7, afeto contínuo, supervisor meta-cognitivo, Soul Mirror). Absorve #218/#224/#314/#315.13/#315.17/#168/#190/#199/#468. Depreca #136. ~2900 LOC, 24 arquivos, 0 dependências externas. | ✅ **7/7 ondas** SESSION_166 | ADR-0060 | commits `4eaf54e..bb464af` (11 commits) | ADR-0060 + 7 ondas |
| 2026-07-21 | **471** | **Cellular BitNet (A.2)** — Micro-atores cognitivos concorrentes em cortex::cellular. CognitiveCell com inbox/outbox MPMC, CellNetwork grafo direcionado, CellScheduler round-robin. 3 cell types: Reasoning, Memory, Perception, Motor. Base para execução concorrente do BitNet. | ✅ SESSION_166 | ADR-0060 Onda 2 | `cortex/src/cellular.rs` + `evolution.rs` | 2b09161 |
| 2026-07-21 | **472** | **Living/Sparse Evolution (A.4/A.5)** — Plasticidade contínua dirigida por entropia em cortex::evolution. Morfogênese neural: spawn/prune células baseado em entropia regional. Bridge com SleepCycle #314 (batch pruning/consolidação) + EWC #314c (proteção) + Synaptic Homeostasis #314d (~18% redução). | ✅ SESSION_166 | ADR-0060 Onda 2 | `cortex/src/evolution.rs` | d428d7a |
| 2026-07-21 | **473** | **Memória Hierárquica L0-L7 (A.1)** — 8 tiers em hermes::memory/: L0 Cache (1KB), L1 Working (64KB), L2 Short (1MB), L3 Episodic (16MB), L4 Semantic (BGE), L5 Procedural (WASM), L6 VFS, L7 Archive (FAT32). Facade MemoryStore com promote_on_read ARC. Baseia-se no 4-tier pipeline #218 + Atkinson-Shiffrin #224. | ✅ SESSION_166 (v2 promote/LRU+LFU) | ADR-0060 Onda 4 | `hermes/src/memory/*.rs` | b3dd9bc |
| 2026-07-21 | **474** | **Afeto Computacional (A.7)** — AffectVector contínuo 5D (incerteza/urgência/fadiga/curiosidade/coerência) em hermes::affect. AffectRegulator modula roteamento. Classifier frontend: EmotionState 7 classes → vetor. Visual: Soul Mirror → jarbas (Onda 7). | ✅ SESSION_166 | ADR-0060 Onda 5 | `hermes/src/affect.rs` | 0b7adb7 |
| 2026-07-21 | **475** | **Supervisor Meta-Cognitivo (A.8)** — ExecutiveSupervisor unificado: Ego Layer (EMA confidence, latência), PonderNet (parada sigmoidal, lambda regularizer), SkillOpt/BitNetTrainer bridges, 7-phase loop. | ✅ v1 (65cec5a) + v2 EgoLayer/PonderNet (bb464af) | ADR-0060 Onda 6 | `hermes/src/executive.rs` + `globals.rs` | 65cec5a..bb464af |
| 2026-07-21 | **476** | **Economia/Compressão (A.6)** — CompressionTier unified enum (1bit→2bit→4bit→5bit→8bit→f16→f32) em k_ai::economy. BudgetManager com tier_policy, temperature, allocate/deallocate. ExpertLifecycleManager com merge/split/stale. | ✅ SESSION_166 | ADR-0060 Onda 1 | `k_ai/src/economy.rs` + `expert_lifecycle.rs` | 4eaf54e |
| 2026-07-21 | **477** | **MoE Dinâmico (A.3)** — Int8Router (local, O(1)) + MoELayer (forward/forward_sequence) + DynamicMoE (birth/merge/split, flush_all, stale/high_entropy trigger, 6 asserts self-test). | ✅ SESSION_166 | ADR-0060 Onda 3 | `cortex/src/moe.rs` | 5326e48 |
| 2026-07-21 | **478** | **MPMC queue** — Ring buffer lock-free multi-producer multi-consumer em k_nano::sync::mpmc. Slots atômicos (EMPTY→STORING→READY→LOADING), CAS loop, power-of-2 mask. | ✅ SESSION_166 | ADR-0060 Onda 0 | `k_nano/src/sync/mpmc.rs` | 4eaf54e |
| 2026-07-23 | **511** | **Emagrecer neural-kernel ADR-0075** — Cutover cirúrgico bin→crates E0–E4. Recon profundo: 29.431 LOC no bin, 12k bin_ahead (41%). Plano: promover cortex/bpe/gguf/agents/neural_fs/boot_logger → crates, depois stub. Audio deferido p/ E4 (ADR-0045). Alvo: ~11.000 LOC (redução 62%). Supersede #467. | ✅ E0 done | ADR-0075 | SESSION_215 | ADR-0075, SESSION_215, BIN_CRATE_DIFF.md |
