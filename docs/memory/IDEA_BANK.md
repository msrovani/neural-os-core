...
| 312f | **Federated learning** — múltiplos AIOS compartilham gradientes (não dados). Agregador central (Hermes Master) combina updates → modelo global melhor | ⏳ Pós-MVP → 🟡 ADR-0081 | v0.85+ | Absorvido pela ADR-0081 — GradientAggregator no Brain Mesh + Hermes Master. **Fase A (transporte P2P R0) + Fase B (skill sync) ✅ SESSION_234** — 2 QEMUs trocam heartbeats + skills via broadcast real |
