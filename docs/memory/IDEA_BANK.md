...
**Última atualização:** 2026-09-26 — s410 mesh 6 OOM bughunt (dedup fnv1a64, caps anti-bloat, I3/I4) + fecho sev audit; #611 ADR-0110 revisada (SESSION_407).
...
| **#536** KernelPack W2A8 via cuda-oxide/kaio (PTX em Rust puro, offline) | **ADR-0105** B0–B2 (+0057 WS-D) | ⏳ Layer S | Host gera pack; runtime = ADR-0105 (FAT→Ready→dispatch). cuda-oxide/kaio = produtor offline; bare-metal QMD + golden. |
...
| **#538** Backlog unificado K³CHJ por custo e anel | ADR-0100 / **ADR-110** | 🟡 fazendo | SESSION_282: filtra residuals 0077–0089; ondas 0–10; TODOs T-001–T-075. Lacuna 0090–0099 (exc. **#539**/0092). **Integração ADR-110**: T-001 (métrica boot) em paralelo a T-017 (USB metal) — anéis diferentes, sem conflito de crate. ADR-110 Fase 1 tasks (@explorer, @fixer, @librarian) mapeiam para ondas Onda 0 (honesty) e Onda 1 (I/O + HardwareInfo) do backlog 0100. Não substitui ADRs temáticas. Integração: gate-conditional RAM embedding de 110 alinha com política de custo×anel de 0100. | |
| **#540** Falcon3 7B alvo; 1B/3B/10B por fit + AirLLM/GGUF | SESSION_291 | 🔄 lab **superseded por #544** (3B-first). Fit/AirLLM permanece; GeneratorPro=7B opcional. | Sem SKU 8/16GB. `llm_boot_plan` + `needs_airllm_at`. |
| **#550** Gates G1–G3 GPU Rust ecosystem + **Multi-ISA NKP1 (SESSION_361)** | **ADR-0105** B0–B4 | 🟡 **parcial** | B0–B3 código ✅ + s386 honesty (ternary só pack; CpuStub refuse; SCORE tri-state). B1.2/B2.4/B4 golden device = AWAITING_HW. |
| **#544** Falcon3-3B Instruct 1.58-bit = lab arquitetural (Falcon3-BitNet Cognitive) | **ADR-0101** | 🟡 Onda 0 parcial | SESSION_298+**387**: header Observe wired; rope 1000042; hub set_model. Residual: metal tok/s; SIMD skip-native. **Integração ADR-110**: ADR-110 reusa `hw_expert_v6` (modelo 3B 1.58-bit) como classificador System 1 no kernel. A escolha de #544 deslocar o lab de 7B-first para 3B-first (IDEA #544) valida a decisão de 110 de reutilizar hw_expert_v6. Ambas trabalham com o mesmo modelo canônico 3B 1.58-bit. Integração: forward pass ternário nativo (ADD/SUB/SKIP) de 110 compatível com o decoder do 3B; gate condicional de RAM (`TOTAL_RAM_MB >= 8192 && HEAP_FREE_MB > 200`) impede bloat. |
...
| **#588** Decisões calibradas (contrato `Decision<T>` + confiança/abstenção + θ da reliableza medida + fim da decisão por substring) | **ADR-0106** / **ADR-110** | ✅ D0–D4 + M0–M4 (s364–s366); residual opcional r3 replay contínuo | s364 contrato+intent+site_policy; s365 typed_sites; s366 JSONL→train HITL×3, Hub `decide`, M4 OUT. Fonte Heuristic até head neural + ECE. **Integração ADR-110**: ambas emergem da premissa AIOS-First (0088). 0106 define o contrato de decisão (Decision<T> com Q8 confidence, margin, source, abstain); 110 decide *o que* classificar System 1 (Laya como classificador hardware). 0106 abstrato: "abster é resultado de primeira classe, não else". 110 prática: "rejeitar garantias absolutas (<35ms forward pass, 150-300MB RAM pinned)". Juntas: 0106 fornece a infraestrutura de como decidir; 110 fornece o que decidir com gates condicionais. Integração: wire Laya classifier output ao contrato Decision<T> (choice: Option<Intent>, confidence: Confidence(u8), margin: Confidence, source: DecisionSource::Neural, abstain: Option<AbstainReason>). |
...
| 312f | **Federated learning** — múltiplos AIOS compartilham gradientes (não dados). Agregador central (Hermes Master) combina updates → modelo global melhor | ⏳ Pós-MVP → 🟡 ADR-0081 | v0.85+ | Absorvido pela ADR-0081 — GradientAggregator no Brain Mesh + Hermes Master. **Fase A (transporte P2P R0) + Fase B (skill sync) ✅ SESSION_234** — 2 QEMUs trocam heartbeats + skills via broadcast real. **Fase B cripto gate L/F ✅ SESSION_240** (HMAC Tier L mesmo range; Ed25519 Tier F externo). **SESSION_306:** revalidação 4c Master/Worker+TOFU (slog ok); split IA/Falcon dual = residual RAM host |
...
| 2026-07-05 | **329** | **Agent.xpu prefill/decode split** — Prefill (processamento do prompt completo) fica na CPU (4 cores paralelos). Decode (geração token a token) vai para GPU via job ring. CPU faz tokenization + embedding, GPU faz matmul. Referência: arXiv 2506.24045. **s328:** decode já yielda via InferQueue; **prefill layer-yield (AirLLM)** = residual candidata sprint seguinte (hiccup prompt) — independente de GPU WS-D. | 🟡 Bloco 28; prefill yield = pós-WS-H | Sprint N+1 (prefill); GPU = Layer S | ~400 LOC |
...
| 2026-07-20 | **467b** | **Compute Dispatch SMP+GPU+NPU** — Camada única de dispatch da LLM (`cortex::compute`): NPU→GPU→CPU-SMP→AVX2→scalar. **WS-A ✅:** wake multi-AP por SIPI direcionado sequencial + stack/PerCpu por-AP + contador unificado + emagrece `neural-kernel::smp` (QEMU `-smp 4` → APs=3, CorePools r0=1 r1=2 r2=1). **WS-B/C ✅ wired:** `parallel_ternary_matmul` (particiona colunas, decode m=1 escala) + dispatcher nos choke points. **WS-D** GPU registra só se `BackendState::Ready` (canário silício); kernel W2A8 = Layer S/HW. **WS-E** NPU XDNA/Intel: detecção PCI + veredito honesto + fallback software (Ring0 MLP CPU, #51); driver/firmware = Layer S/sponsor. **WS-H ✅ (s328):** InferQueue Full D+B+C (UI viva). **Decisão s328:** NÃO empacotar WS-D/E no entregável WS-H — job já usa `dispatch_ternary`; sem Ready/pack/FW = zero ganho; Prefill AirLLM = sprint seguinte; CPU W2A8 (#485) ≠ GPU WS-D. | 🟡 WS-A/B/C/H ✅; WS-D/E = Layer S/HW | ADR-0057 | SESSION_328 | `cortex/{compute,infer_queue}.rs`, `k_nano/smp`, `k_hal/{npu,gpu/compute_dispatch}` |
...
| 2026-08-22 | **537** | **Migrar callers put_kv→NSGDB primário (ADR-0091 Fase 3)** — s385: put_doc+put_kv sync_write + CRDT version OK (dual-write glue). Residual: 75 callers ainda escrevem Tickv primeiro; cutover = write primário NSGDB. | 🟡 parcial s385 | ADR-0091 | SESSION_284+385 | hermes/neural-kernel callers |
...
| 2026-08-09 | **540** | **GGUF i2_s/TQ2_0** — **✅ s309 IDs; ✅ s358 ggml type 35; ✅ s371 layout 66B/256** (PrismML 24/32 aposentado). Falcon3-1.58bit arquivo costuma ser BF16. | ✅ | - | SESSION_309→358→371 | `gguf.rs` |
| 2026-09-03 | **549** | **Falcon3 GGUF e2e: tokenizer real + KV-cache 32K + metadata keys** — (a) vocab=32678 do GGUF caiu p/ fallback 256 (chave metadata divergente); alinhar nomes de chave GGUF (`general.architecture`/`llama.*`) p/ ler `intermediate_size` (veio 6144, deveria 9216) e vocab corretos; (b) SentencePiece BPE (`bpe.rs` já tem `encode_sp32`) no caminho GGUF; (c) KV-cache 32K (`kv_h2o.rs` tem eviction) — hoje cap 512. Objetivo: prompt → tokens → logits → resposta de verdade no boot. | 🟡 agendado | - | SESSION_309 (residual) | crates/cortex/src/gguf.rs, crates/cortex/src/bpe.rs, crates/cortex/src/kv_h2o.rs |
| 2026-08-25 | **539** | **Memória persistente no pendrive — FileFlash + nsgdb_bridge real no bare-metal** - (a) neural-sgdb v1.1.13 agora compila limpo em x86_64-unknown-none (upstream 3cc3010 gateia SSE2/AVX) → reabilitar nsgdb_bridge real no kernel (hoje stub). (b) FileFlash: FlashController sobre NSGDB.BIN pré-alocado no FAT32 do stick (padrão BOOT.LOG data-only), montado via plano boot_bind NVMe>AHCI>ATA>USB — mata o fallback RAM volátil do TickvLite. (c) sgdb_ingest_bootlog: ramlog → L3 episódico Durable pós-boot_init; boot seguinte faz recall e APRENDE (fecha Remember entre boots, ADR-0088). Pendrive = memória portátil compartilhada OS↔host MCP (mesmo db nos dois lados). | 🟡 fazendo | ADR-0091 | SESSION_292 | crates/k_nano/src/storage/{flash,tickv}.rs, crates/k_ai/src/sgdb/{mod,nsgdb_bridge}.rs, tools/mkfat32.py |
| 2026-08-29 | **541** | **Compositor hot path (desktop lento)** — `fill_rect` era set_pixel/pixel (~1M/frame); `fill_rect_fast` bpp=4 `aw/4` pintava 25%; glow O(r²) sqrtf (~280k); `TARGET_FRAME_TICKS=3` + PIT 18 Hz ≈ 6 FPS e frame caro faminto o scheduler. Fix: scanline + doubling memcpy, glow isqrt, 1 frame/tick, orb menor, dock 1×. ADR-0090 afirmava glow integer-only — mentia. | ✅ SESSION_294 | — (fix pontual / ADR-0058) | SESSION_294 | `jarbas/src/display/{fb,compositor,soul_mirror,agent}.rs` |
| 2026-08-29 | **542** | **Mouse USB tablet QEMU** — ✅ s382. Ver append SESSION_382 (sem dup). | ✅ | - | SESSION_382 | xhci, InputAgent |
| 2026-08-30 | **543** | **TTS streaming por frases** — split_into_sentences() divide texto por pontuação, sintetiza cada frase separadamente via Piper TTS. Reduz latência percebida de ~500-2000ms (bloqueio completo) para ~50-200ms (primeira frase). StreamingTtsState: Idle | Streaming { buffer, pos, queue }. Tick usa core::mem::replace para ownership-safe reassign. | ✅ | ADR-0093 (Soul/Persona) | SESSION_292 | crates/jarbas/src/audio/jarvis.rs |
| 2026-08-31 | **544** | **Falcon3-3B Instruct 1.58-bit como laboratório** — shift DeepSeek V4-Flash / 7B-lab → *small reasoning engine* no 3B. Onda 0 = packing v6 + kernel ADD/SUB/SKIP no `x86_64-unknown-none` (hoje metal=scalar; AVX2 host=FMA f32). **s387:** `set_model_header` Observe + rope 1000042 + GGUF rope_precompute + hub Active→set_model. Residuals: metal tok/s; #549. | 🟡 | ADR-0101 | SESSION_298→387 | `cortex/{bitnet_*,nn,model_fit,model_hub,gguf}.rs`, `tools/convert_falcon3_*.py` |
| 2026-08-31 | **545** | **Extrair kernel_boot em módulos** — main.rs 4777 LOC monolítico. Cada SESSION adiciona 50-100 linhas. Proposta: boot_memory.rs, boot_storage.rs, boot_models.rs, boot_greeting.rs (~500 LOC cada). Reduz dívida técnica e facilita debug. | ⏳ | - | SESSION_299 | `crates/neural-kernel/src/main.rs` |
| 2026-08-31 | **546** | **Gate QEMU smokes com feature flag** — 40+ boot_smokes irrelevantes para QEMU dev/test (wifi, btrfs, luks, ext4, bluetooth). Feature `#[cfg(feature = "full-smokes")]` ou `if !qemu`. Boot QEMU 30% mais rápido. | ⏳ | - | SESSION_299 | `crates/neural-kernel/src/main.rs` |
| 2026-08-31 | **547** | **Cross-boot NSGDB recall** — Com ATA habilitado em TCG, BOOT.LOG grava no FAT32. Segunda instância QEMU lê via `ingest_bootlog()` e faz recall de inferências. Goal: segunda instância processa dados do boot anterior. | 🟡 | ADR-0091 | SESSION_299 | `crates/k_ai/src/sgdb/nsgdb_bridge.rs`, `crates/k_ai/src/boot_observe.rs` |
| 2026-09-01 | **548** | **Ring3 sandbox B/C — recusa Fuchsia process-OS** — rascunho 0102 (Job/Handle/ExceptionChannel + SYSCALL seL4, 16 sem) rejeitado. Alvo = um sandbox CPL=3; wasmi default; CapGate bitflags; `int 0x90` até GDT+GS+EFER. Honesty: destubar P6 + `ring3 = ["k-nano/ring3"]`. Execução = ADR-0100 T-051–T-057. Não substitui ADR-0077. | 🟡 | ADR-0102 | auditoria 2026-09-01 | `k_nano/src/paging.rs`, `isolation_ring.rs`, `user_mode.rs` |
| 2026-09-06 | **550** | **VectorWare "Rust SIMD on the GPU" — referência warp↔SIMD** — `core::simd` mapeado em lanes de warp (N=32): shuffle=reduce_sum, vote/ballot=Mask::any/all, swizzle=shuffle; zero-custo só se N==largura do warp (NVIDIA 32, AMD 32/64); interpretador determinístico do IR no host + differential testing GPU↔host (mesma doutrina nossa de port Rust-exato, SESSION_247/248). NÃO acionável: exige fork do rustc deles (PTX + std-on-GPU); portable_simd nightly instável; nosso target soft-float nem split SSE/AVX aceita (SESSION_249b). Valor = modelo mental p/ kernels GPU reais (ADR-0087) + regra N=32 no W2A8 futuro. | 💰 referência | ADR-0087 | estudo 2026-09-06; SGDB `md/L3/mcp/1788713786925000` (entidades `gpu/simd-warp`) | — (docs) |
| 2026-09-09 | **551** | **Escada de instrumentos FB (bisector canônico HW real)** - serial é invisível no metal; cada build adiciona UM instrumento lock-free/volatile no FB: diag_mark (barras) → diag_stamp_agent → diag_stamp_exception (EXC_FB_FN) → tick_stage (TICK_STAGE_FN) → heartbeat T no timer IRQ (vivo-vs-morto) + dígito S<n>. Bridges registrados no claim_graphics. **✅ SESSION_316:** suíte completa s318→s326; freeze @1370 network_agent ainda aberto aguardando discriminador heartbeat. | ✅ | - | SESSION_316 | `jarbas/src/display/fb.rs`, `agent-core/src/lib.rs`, `k_nano/src/interrupts.rs` |
| 2026-09-09 | **552** | **xHCI Port Power pós-HCRST + hub enumeration (estudo Redox)** - HCRST pode deixar PP=0 → CCS lê 0 para sempre ("nenhuma porta CCS" no metal; QEMU mantém PP=1 e mascarava). Fix: RMW PP=1 em todas as portas pós-run + dump PORTSC pre/pos. Residual: stick atrás de hub interno exige hub enumeration (class requests ao hub device, padrão usbhubd do Redox; route string no slot context bit 26 + NumberPorts slot.b; TTT/MTT). | 🟡 | ADR-0062 | SESSION_316 (lib-1) | `k_nano/src/xhci/mod.rs`, `k_hal/src/usb/hub_msc.rs` |
| 2026-09-09 | **553** | **Cap no loop smoltcp poll ZERO-delay** - poll()/dhcp_poll() giravam para sempre com poll_delay()==ZERO (socket com trabalho cuja resposta nunca sai: TX degradado em HW real + tráfego LAN real broadcast/ARP/mDNS; QEMU/slirp nunca entrega). Fix: cap 64 iterações + contador POLL_LOOP_CAP (AIOS: degrada, não congela). Residual: caçar POR QUE o TX degrada no metal (TX DD nunca seta?). | 🟡 | - | SESSION_316 | `hermes/src/netstack.rs` |
| 2026-09-10 | **554** | **Doc-drift top-of-repo — métricas/versões/IDs derivam** - TECNOLOGIAS/README/TODO/INDEX propagavam números velhos (~26K LOC / 180+ arquivos / ~50 agentes / v1.9.5) vs real medido (~148K LOC / ~671 .rs / 41 seeds / s328). Corrigido por medição (Cargo.toml members + `.rs` + `skills/agents` + `cargo test --no-fail-fast`), + Falcon3 dims 22L/3072/vocab 131072, + boot=Limine, + Agency data-driven, + IDs duplicados (2.10/2.10g/9.4) e INDEX de ADRs (0090/0091/0093/0094 ausentes). **Regra: doc não é fonte — medir o repo.** | ✅ | - | SESSION_329 | `TECNOLOGIAS.md`, `README.md`, `TODO.md`, `docs/architecture/INDEX.md` |
| 2026-09-10 | **555** | **Licença inconsistente (AGPL vs MIT)** - `LICENSE` = AGPL-3.0 e README = AGPL-3.0, mas `TECNOLOGIAS.md` §15 declara "código próprio MIT". Decisão do maintainer pendente; **não** alterar texto de licença sem OK explícito. | ⏳ | - | SESSION_329 | `LICENSE`, `TECNOLOGIAS.md` |
| 2026-09-10 | **556** | **Cadência de tick adaptativa (ADR-0104)** - o alvo do LAPIC era `TARGET_TIMER_HZ=60` hardcoded (bypass da premissa máxima 0088: detecta mas não decide). Fix: R0 mede (LAPIC counts/s via CURRENT_COUNT×TSC, jitter, alive) + `set_tick_hz`; R1 `k_hal::timer_cap` sanciona a faixa `[max(30,counts/u32MAX), min(240,counts/INIT_FLOOR)]` e decide com rails {30,60,120} + dwell 5s + confirmação 2 janelas + rollback (auto só mantém/desce; subir>default = HITL); R3 jarbas `target_frame_ticks()`/`frame_cost_us`; bin dirige + `/tick` + `CONFIG.TXT TICK_HZ=`. **Integração ADR-110**: mesma filosofia "detect→measure→decide→optimize→version" da AIOS-First (0088). 110 rejeita garantias absolutas de forward pass (<35ms) assim como 0104 rejeita alvo de tick constante. Ambas usam medição empírica em vez de números mágicos. 110 gate-conditional RAM embedding (`TOTAL_RAM_MB >= 8192 && HEAP_FREE_MB > 200`) complementa a política de faixa sancionada de 0104: 110 decide *se* embedar modelo por RAM; 0104 decide *a* taxa do tick por medição. Integração: ciclo compartilhado detect→measure→decide→optimize→version. | ✅ | ADR-0104 | SESSION_330 | `crates/k_nano/src/{apic,interrupts,boot_report,boot_mode}.rs`, `crates/k_hal/src/timer_cap.rs`, `crates/jarbas/src/display/{compositor,agent}.rs`, `crates/neural-kernel/src/{main,shell}.rs` |
| 2026-09-11 | **557** | **Orb v2 MCU-JARVIS + Hub Health panel agent-driven** - fps era capado por LCM acidental de 2 gates de tick (`%3` × `hz/30` = 15 fps); animação frame-count; cor em serrilhado; paleta morta por mismatch de string; 2 divisões/px; FFT ~129k trig/buffer. Fix: gate wall-clock 30fps + `OrbState` enum com sinais reais + paleta em rails + halo SWAR sem divisão + `ring_spans` LUT + LOD (bench 112-130 µs/frame); FFT Goertzel (tap cap 1024); painel **agent-driven** (`HubHealthAgent` hermes decide, compositor só desenha; dados medidos, `n/a` ≠ 0; F12/orb click/badge 4 LEDs). | ✅ | - | SESSION_331 | `crates/jarbas/src/display/{soul_mirror,compositor,fb,agent,gauges,shortcuts,avatar}.rs`, `crates/hermes/src/hub_health.rs`, `crates/k_nano/src/{interrupts,xhci}` |
| 2026-09-16 | **#558** | **Playback paceado pelo clock do áudio** - mixer vs HDA. ✅ `compute_mixer_want` + TSC (s368/B3.1). Residual aceite metal `PLAY_SAMPLES_DROPPED==0` → #558b. | ✅ | ADR-0104 | SESSION_352→368 | `mixer.rs`, `hda.rs` |
| 2026-09-16 | **#559** | **Duplexidade / barge-in** - ✅ s381 `request_interrupt` sem `cancel_active`. Residual half-duplex/AEC → #559b. | ✅ parcial | - | SESSION_352→381 | `voice.rs` |
| 2026-09-16 | **#560** | **`STT_UNCERTAIN` falado** - ✅ DisplayAgent consome (toast+chat). | ✅ | - | SESSION_352→381 | `stt.rs`, `display/agent.rs` |
| 2026-09-16 | **#561** | **Teto utterance** - ✅ pcm_buffer cap 30s (s381). Residual CTC incremental (matriz logits) → #561b. | ✅ parcial | - | SESSION_352→381 | `voice.rs`, `stt.rs` |
| 2026-09-16 | **#562** | **Áudio stream SPSC** - ✅ s383 `MicFrameRing` (VOICE+WAKE); EventBus só VAD/controle. | ✅ | - | SESSION_383 | `jarbas/audio/mic_ring.rs` |
| 2026-09-16 | **#563** | **Wake-word de verdade (janela, features, corpus)** - (1) o MLP lê `energy_history[..16]` = os 16 mais **VELHOS** de uma janela de 64 frames (1,28 s) e só é avaliado no fim dela → latência de detecção até 1,28 s e o classificador nunca vê o "JARBAS" recém-dito; (2) entrada é RMS pura (sem espectro) → o detector real é o padrão de 2 picos; (3) o "98,4% (2000+8000 amostras)" não tem script nem dataset no repo (`tools/` só STT/Piper) = overclaim. Fix: janela deslizante dos últimos 16 + avaliação a cada 4 frames (≤80 ms); `tools/train_wakeword.py` com log-mel + confusáveis PT-BR ("serviço", "alvará", "já vi", "jarra"); FPPH em holdout disjunto. | 🟡 | - | SESSION_352 | `crates/jarbas/src/audio/wakeword.rs`, `tools/` |
| 2026-09-16 | **#564** | **Orçamento de latência de voz (`VOICE_LATENCY`)** - nenhum estágio mede wake→VAD start→end→transcrição→1º token→1ª amostra TTS→alto-falante, e o endpointing atual (5 frames p/ entrar ≈100 ms; 15 p/ sair ≈300 ms) não tem número para julgar. O projeto já tem TSC/telemetria (`frame_cost_us`, InferQueue): publicar deltas por estágio + linha no HUD. Alvo proposto: wake→1º áudio ≤1,5 s (LLM domina) e endpoint→transcrição ≤400 ms. Sem isso não há guarda de regressão para "fluidez de voz". | ⏳ | - | SESSION_352 | `crates/jarbas/src/audio/*`, `crates/jarbas/src/display/gauges.rs` |
| 2026-09-16 | **#565** | **Contexto de diálogo com orçamento + memória episódica** - `deliver_transcript` prefixa os últimos 3 turnos sem limite e o LLM clampa ctx (512 no Falcon3 1,58-bit do lab; 4096 heavy) → turnos longos truncam **a pergunta atual**. Fix: orçamento por chars/tokens descartando os turnos **mais antigos** primeiro, nunca o corrente. E o histórico é RAM-only (10 turnos): gravar em SGDB episódico (ADR-0063) para sobreviver a reboot (premissa AIOS: memorizar). | ⏳ | ADR-0063 | SESSION_352 | `crates/jarbas/src/audio/voice.rs` |
| 2026-09-16 | **#566** | **Identidade da voz (`voice_input`) + HITL p/ skill privilegiada** - `voice.rs` publica `USER_INTENT`/`STT_TEXT` com `CapabilityToken::Legacy(1)`: um comando falado é indistinguível de evento interno confiável, e skills privilegiadas (instalar/formatar/shell) podem ser disparadas por voz sem confirmação nem verificação de locutor — contra a doutrina "Trust por `(token, agent, skill)`". Fix: token de agente `voice_input` + confirmação HITL para skill privilegiada; verificação de locutor (voiceprint) fica 💰 (depende de modelo). | 🟡 | - | SESSION_352 | `crates/jarbas/src/audio/voice.rs`, `crates/hermes/src/` |
...
| 2026-09-19 | **#585** | **Mesh WHPX 6-node + orb** - lab STATIC+L2 hub; MESH_HEALTH→orb/roles; HubHealth=SystemInfo; HDA QEMU. ✅ hub script + UI; residual peer B. | ✅ | ADR-0081 | SESSION_360 | `mesh.rs`, `soul_mirror`, `hub_health`, `qemu_l2_hub.py` |
| 2026-09-19 | **#586** | **Fail/warn dmesg honesty** - TLSPINS FAT OK=`fail`; Trust diagnostic→Crashed; CapGate PoC=`warn`. ✅ slog ok; trust_allow+skip; CapGate DENY=`ok`. | ✅ | ADR-0092 | SESSION_360 | `gguf.rs`, `main` SystemAgent, `cap_gate`, `jarbas_fb` |
...
| 2026-09-21 | **#596** | **event-bus honesty** - VecDeque ilimitada (SESSION_352); publish Ok fantasma; Latent recv_count no enqueue; as_legacy(Ed25519)→1; capacity(0)→1; libm/dedup mortos. ✅ depth+drop_oldest; Ok(n); recv=try_receive; as_legacy→0; trim. Residual: AUDIO ring SPSC (#562). | ✅ | ADR-0088/0092 | SESSION_375 | `crates/event-bus` |
| 2026-09-21 | **#597** | **Upstream soft-float bumps** - ✅ spin 0.12 (s383) + LazyLock (s384) + smoltcp 0.14 (s384). ❌ x86_64 0.15 bloqueado: nightly-2026-07-05 (1.98) sem `Step::*_overflowing` (0.15 exige ≥1.99; pin 1.98 irrevogável). | ✅ parcial | — | SESSION_383→384 | Cargo.toml |
| 2026-09-21 | **#598** | **neural-kernel wire honesty** - dual shutdown/BootLog; link_watcher/email/rss órfãos; CapGate DENY=`info`; Cargo TLS/UART/allocator duplicados. ✅ soft→k_ai; BootLog Agent→k_ai+bridges; delete orphans; slog ADR-0092; Cargo trim; codemap. Residual: main.rs split (#L3). | ✅ | ADR-0057/0088/0092 | SESSION_376 | `neural-kernel` shutdown/boot_log/Cargo |
| 2026-09-21 | **#599** | **wasmi 2.x no hermes** - ✅ s384 wasmi 2.0 (`validate`+btree+libm); `instantiate_and_start`; 205 tests. | ✅ | ADR-0059 | SESSION_384 | `hermes/Cargo.toml`, `wasmi_rt.rs` |
| 2026-09-21 | **#608** | **Logging ADR-0092 honesty** - COM1 presente → slog não ia ao BOOT.LOG; log() duplicava; CapGate DENY warn; fetch=FAIL sev ok; Net tick spam. ✅ dispatch to_file→append_raw; log=slog only; DENY/ok; FAIL→fail; tick→trace; shutdown FAT flush. Residual: slog info restante nos drivers. | ✅ | ADR-0092/0088 | SESSION_389 | `serial.rs`, `boot_logger.rs`, CapGate, Net, Market, UPDATE |
| 2026-09-21 | **#607** | **ModelHub + Trinity honesty** - register_model drop; get_or_mmap morto; Falcon3→Agent; generate_from_slot Active vazio; CapGate teatro Gpu/Display; Vision fingia LLM. ✅ set_*; weight-only; Active size; CURRENT_MODEL path; Block→DeviceIo; Vision fora. Residual: mmap FAT→PackedTernary (#607r). | ✅ | ADR-0085/0083/0101/0059 | SESSION_388 | `model_hub.rs`, `trinity.rs`, `trinity_inject.rs`, `cap_gate.rs` |
| 2026-09-21 | **#600** | **CapGate tokens DynSkill** - ✅ s383 `required_tokens=[0xD1]` + `register_dynskill` trust_allow; Legacy(1) deny; Hermes remap só se trusted. | ✅ | ADR-0052 | SESSION_383 | `dynskill.rs`, hermes |
| 2026-09-21 | **#601** | **skill-registry honesty** - WASM execute teatro; Trust Contain bypass; cache all Ok; list_skills blob; FanOut/index/task mortos; 0 testes. ✅ fail-closed wasm; check_or_cache; idempotent cache; SkillListEntry; unregister; trim; 10 tests. Residual #599. | ✅ | ADR-0052/0059/0088 | SESSION_377 | `crates/skill-registry` + hermes |
| 2026-09-21 | **#602** | **spin vs ticket-lock policy** - ✅ s383 documentado em `ticket-lock/codemap.md`. Bump versão = #597. | ✅ | SESSION_316 | SESSION_383 | ticket-lock codemap |
| 2026-09-21 | **#605** | **hermes honesty s379** - evolve rollback candidato; CapGate Allow+stubs Ok; TLS ready sem bridge; ApprovalGate deny invisível; matrix "Aprendi"; signed=true; session_load dup; wasmi yanked. ✅ hot_swap live/prev; trap unwired; TLS deny; resolution; draft HITL; signed real; clear HNSW; 0.47.2; 203 tests. Residual #599. | ✅ | ADR-0059/0088/0092 | SESSION_379 | `crates/hermes` |
| 2026-09-21 | **#542** | **Mouse USB tablet QEMU** - ✅ s382 deferred HID. Residual EnableSlot TCG. | ✅ | - | SESSION_382 | xhci, InputAgent, DisplayAgent |
| 2026-09-21 | **#606** | **jarbas honesty s381** - ✅ energy/slog/barge-in/pcm/VGPU. Residual #597 (bumps). #542/#562 ✅. | ✅ | ADR-0058/0088/0092 | SESSION_381→383 | `crates/jarbas` |
| 2026-09-21 | **#604** | **hermes host ABI wire** - ✅ s380. Residual #599. | ✅ | ADR-0059 | SESSION_380 | `wasmi_rt`, orphans |
| 2026-09-21 | **#603** | **ticket-lock honesty** - ✅ s378. Residual #602 → ✅ s383 policy doc. | ✅ | ADR-0088 | SESSION_378 | `crates/ticket-lock` |
| 2026-09-21 | **#595** | **k_ai R2 honesty residual** - recovery `executed` falso; I3 Pass proxy; Trust path default-allow; token budget teatro; Trainer residual; slog hw/FS/BQ TRACE; stubs ReAct/MCP. ✅ status honesto; I3 Warning+hermes; record_tokens; residual FFN/Attn; fail-closed stubs; trim skill-registry. | ✅ | ADR-0088/0092 | SESSION_374 | `k_ai` self_heal/trust/cognitive/sgdb + hermes safety |
...
| 2026-09-22 | **#609** | **MHI bughunt fix-1/2/3** - buddy contava pedido (não bloco); free assimétrico + OOB; resize_heap pool errada; write-back morto; pisos literais; lba-como-RAM; predict 0 ambíguo; record_access morto; stubs mhi_scheduler dup. ✅ 1<<o simétrico+clamp; resize del; write-back del; consts LOWMEM/pisos; facade pub use; talc 4.4.3/x86_64 0.14 PINADOS. Toca #328 (buddy), #420 (MHI DMA wiring), #537 (tier0), 282h (auto-migrate segue ⏳). | ✅ | ADR-0087 | SESSION_393 | `vram.rs`, `allocator.rs`, `mhi.rs`, `cache.rs`, `memory.rs`, `fat32.rs`, `global_arena.rs`, `memory_agent.rs` |
| 2026-09-22 | **#610** | **BEI bughunt ADR-0060 (8 ondas)** - MPMC CAS infinito; merge_pair sem pesos; observe() morto; lifecycle gate; pressure() isolado; L0 clear/auto_promote stub/L7 sem teto; spam ProceedWithBudget; sev Trace; MPMC dup; lock-order pc→net. ✅ MAX_RETRIES; get_weight média; feed_plasticity; gate is_empty; heap_observe; TTL+sweep+promote+1GiB cap; LAST_BUDGET; ok/warn/fail; facade sync; low_mem gate; wasmi portable-dispatch confere. Pendências: talc 5.x ⏳ (API quebra); x86_64 0.15 ⏳ (nightly pin); TL2 BitNet 💡 port. | ✅ | ADR-0060 | SESSION_394 | `sync/mpmc.rs`, `mpmc.rs`, `moe.rs`, `cellular.rs`, `evolution.rs`, `economy.rs`, `bei.rs`, `memory/mod.rs`, `hal_offer.rs`, `memory_store.rs` |
| 2026-09-25 | **#611** | **Laya System-1 real (pós-revisão ADR-0110 + adendo §10)** - reuso de `hw_expert_v6` como classificador de intenção = erro de categoria (S248, gate #492). Laya real verificado (Convai, ModernBERT 421M, Apache-2.0): **842MB fp16, 193-464ms CPU, zero-shot quase ao acaso (ECE 0,466)** → rodar o modelo no metal REJEITADO. ✅ Rota aprovada = **destilação do contrato**: promover `cortex::intent_decide::decide_intent` (15 arms hand-weighted) → trained em BitNet pequeno (~1M params, classe hw_exprt, loader v6 existente; zero motor novo), saída `Decision<Intent>` (0106 = primitivas boolean/choice/score do Laya). ⏳ exige: dataset pacote/intenção→rótulo NÃO-circular, holdout disjunto, forward Rust-exato + validação do artefato (S247/248), gate ≥65% específico + ECE calibrado, evidência `docs/evidence/laya-zero/`, budget TSC no tick. ✅ trivial: NoProto `TaskType::LayaIntent=8`. ❌ rejeitados: TickV-TTL (→ring RAM S352); skip-detectores; drop sem chokepoint; fast-path sub-tick sem evento-drivado (tudo ~55ms hoje). | ⏳ | ADR-0110/0106 | SESSION_407 | `intent_decide.rs`, `trinity.rs`, `decision.rs`, `noproto.rs`, `hermes/security.rs` |
| 2026-09-25 | **#612** | **Formato BITCOS-like por tensor + seleção automática** - formato por-tensor com seleção automática de quantização (paper real, ainda sem código no repo). Cadeia: ADR-0084 (fidelidade 2B4T) → ADR-0085 (v6) → ADR-0101 (lab 3B; 10B usa 40L/23040, não 22L) → ADR-0057 WS-D/E (GPU) → #611 (System-1). Estudar paper antes de qualquer código. | ⏳ | ADR-0084/0085/0101 | SESSION_406 | `cortex/model.rs`, `tools/convert_*` |
| 2026-09-25 | **#613** | **KV-INT8/paginado** - KV cache quantizado INT8 + paginação; pré-requisito do Bonsai no metal (KV hoje f32 por layer, 603MB p/ 3B ctx 4096). Mesma cadeia #612. | ⏳ | ADR-0085/0057 | SESSION_406 | `infer_queue.rs` (KvCache), `cortex.rs` |
| 2026-09-25 | **#614** | **2B4T como referência permanente** - MIT, 2.4B, referência de fidelidade do pipeline (forward/quant/ativação). Já coberto por ADR-0084/0085. | ✅ | ADR-0084/0085 | SESSION_406 | `models/`, ADRs |
| 2026-09-25 | **#615** | **JEV-local/AgentJev-0.6B como backend SystemOneDecisionEngine** - modelo pequeno local como backend do decide (cruza com #611 destilação: mesmo contrato `Decision<Intent>`, outro motor). | 💡 | ADR-0110/#611 | SESSION_406 | `intent_decide.rs` |
| 2026-09-25 | **#616** | **Correção JEV §5** - pesos públicos existem; alinhar spec §5 ao que os pesos realmente implementam. Pré-requisito prático de #615. | ⏳ | ADR-0110/#611 | SESSION_406 | docs JEV |
| 2026-09-26 | **#617** | **Bonsai-8B como alvo Pro (gate + dependências)** - `prism-ml/Ternary-Bonsai-8B-gguf` verificado: Apache-2.0, 8.19B, 36L, GQA 32Q/8KV, ctx 65536, Q2_0 g128 2.125bpw, 2.03 GiB (F16 16.38 GB). ⏳ exige: origem canônica só `prism-ml` (há duplicatas) + NOTICE/proveniência checados; Q2_0 proprietário NÃO roda em llama.cpp mainline (fork `PrismML-Eng/llama.cpp` ou conversão TQ2_0 + parity contra arquivo real, S309); KV-INT8/paginado (#613) antes — 16K precisa 5–5.5 GiB; 2B4T (2.4B, MIT) segue referência (#614). Cadeia: ADR-0084/0085 → ADR-0101 (lab 3B) → ADR-0057 WS-D/E → #613 → este alvo. | ⏳ | ADR-0085/0101/0057 | SESSION_409 | `gguf.rs`, `tools/convert_*`, `model_hub.rs` |
| 2026-09-26 | **#618** | **Mesh runtime hygiene: caps de aprendizado + loop de feedback** - mesh_knowledge dedup VectorClock morreu no campo (clock.tick() por TX → dedup nunca derrubava → RX MEM 648× → OOM ~11KB/tick). ✅ s410: fingerprint FNV-1a TX+RX (RX 648→2, workers estáveis T+122k+); caps skill_observer 128 / approval 64 / marketplace 32; loop I4→LLM cortado (sched Violation/Warning não escala — era eco da própria decisão, 7× amplificação Master); I3 fantasma → note_trust_entries (push pattern); sev audit fecho s409+s410 (23 subs → Ok). Residual ⏳: re-test Master pós-fix; checklist canônico criado (s410d, doc runtime-hygiene-checklist.md). | ✅ | ADR-0081/0092 | SESSION_410 | `mesh_knowledge.rs`, `runtime_observe.rs`, `skill_observer.rs`, `approval.rs`, `skill_marketplace.rs`, `safety_invariants.rs`, `slog.rs`, `memory_systems.rs`, `trust.rs`, `fts_search.rs`, `skill_gen.rs`, `skill_opt.rs`, `wasmi_rt.rs`, `gpu/backend.rs` |
