...
|--------|--------|-------|--------|----------------------|
| 234 | ADR-0081 | Mesh P2P real + migração R0 | P2P entre 2 QEMUs + transporte→k_nano | **Dois kernels AIOS se descobrem e trocam skills via rede real** (e1000 + UDP 42069): TX heartbeat em ambas, RX cruzado (A recebeu clock=4796 de B), Master push 15 skills (broadcast=true), Worker apply via poll_p2p. Migração arquitetural (oracle): transporte+serviço mesh → k_nano R0 (`p2p_tick`, `udp_broadcast::send/recv`); non-heartbeat → EventBus `P2P_PACKET`; net_bridge P2P removido; bin re-exporta statics NIC de k_nano. skill_sync/marketplace = R3 (hermes) via subscribe/poll. `set_nic_config` só pós-config (driver-init → sandbox TX). Commits f240fa4 + 0eec18f. 0 erros. |
