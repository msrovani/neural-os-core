...
|--------|--------|-------|--------|----------------------|
| 235 | ADR-0081 | Mesh apps reais 1+2+3 | Marketplace real + PROMOTE + Papéis | Marketplace popula SKILL_REGISTRY (14 skills) e broadcasta offers reais (throttle TIMER_TICKS — scheduler rate-limited fazia 200 CALLS demorar minutos). PROMOTE Worker→Master 'PROMOTE\0name\0desc'. Papéis: assign_roles envia 'ROLE\0target\0role_u8', receptor aplica set_role. Fix eleição: lazy-init usava MAC vs peers [source_id,..] → todos Worker; agora [node_id(),0,0,0,0,0]. VALIDADO 2 QEMUs: A=Master (15 push+14 offers), B=Worker (RX type=4 + role Memory). Commits 9239ac9/e4917c1/50bdf6b. |
