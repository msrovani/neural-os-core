...
- **ADR-0059** Runtime App Factory 🟡 Proposed: runtime WASM real **wasmi** (no_std) + geração validada por IA (WAT/DSL sob gramática #412 → assembler → sandbox-teste → promover `agent-wasm` ADR-0052); supersede ADR-0031(WASM)/0032; aposenta VM `Op` custom; Rust→wasm on-device = residual (sem rustc)
