...
- **Falcon3 lab tok/s + OOM UI (SESSION_349/351):** script canónico `tools/measure-falcon3-toks.ps1 -Accel whpx`. WHPX ≈0.15–0.16 tok/s — ler `milli=`/`us=`, não `decode_tok/s` inteiro. FAT size pode ser 7B mal-nomeado → **sempre** `v6_file_size` do loader; parse in-place (sem leak 2×). Header `max_seq=32768` (Instruct denso) ≠ ctx 1.58bit (4096) → clamp no loader + ctx≤512. `Tensor::new` checked_mul **não basta** — `vec![0.0; a*b]` ainda OOM; usar `f32_zeros`/`try_reserve`. Cap GlobalAlloc 256 MiB = falso positivo no load (~315 MB); refuse só se size > janela bump (~2 GB). Pós-bench: `capacity overflow` raw_vec @T+800 (aberto). HW: `PACK_LLM=all python tools/build_image.py --hw --unified --build-boot --size 8192` → `target/usb_hw.img`.
