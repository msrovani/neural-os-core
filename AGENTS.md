...
- **heap_start desalinhado → tail da região crescida desmapeado → #PF no fim do boot (SESSION_258, confundido com o wrap 2⁶⁴):** o smoke do CI travava APÓS `auto-grow 512→768MB` com #PF em `rep movsq` (CR2 = 1 página além do fim mapeado, `k_ai::sgdb::art::insert_rec`) — diagnóstico empírico via `dump_pt_walk`/pfwalk (heap-trace): pte da última página do grow **não presente** no momento do fault, mesma CR3. Causa raiz dupla: (1) o padrão `*(.bss .bss.*)` do linker **engolia `.bss.heap`** → HEAP_BUFFER voltava para o `.bss` comum em endereço desalinhado (ex. `0xffffffff809fd698`); (2) com `heap_start` não-alinhado, `heap_start + HEAP_LIMIT` caía no meio de uma página que o grow nunca mapeava → os últimos ~0x698 bytes da região eram usáveis pelo allocator mas não mapeados. Fix: seção renomeada p/ `.heap` (NOLOAD — rustc emite custom non-`.bss*` como PROGBITS, 512MB no arquivo senão) + `ALIGN(MAXPAGESIZE)` + `grow_bump_auto` cobre a página do último byte pedido (`(extra + base_off).div_ceil(4096)`). O wrap 2⁶⁴ (offset ~2044MB, 2B) **continua aberto** — este fix resolveu o stall do boot smoke, não o wrap. Regra: `HEAP_BUFFER`/qualquer seção-alvo de extensão por mapeamento precisa de base page-aligned; seção própria de bss custom deve usar nome `.bss*` (NOBITS) ou `NOLOAD` — e conferir com `llvm-readobj --sections` o tipo real (PROGBITS vs NOBITS) após mexer no linker.
- **`map_page_direct` deve retornar bool e NUNCA no-op silencioso (SESSION_258, auditoria #1):** o early-return em HUGE_PAGE (e3/e2/e1) fingia sucesso — o grow "verificava" com `heap_pte_present` (que vê a entrada gigante como present) e avançava `HEAP_LIMIT` sem páginas reais. Fix: contrato verificável — retorna `true` SÓ se a página ficou present+writable para o `phys` pedido; HUGE_PAGE validada por cobertura (`huge_page_covers`); PTE já presente com outro phys = conflito → `false` (nunca alias silencioso). Call sites (grow/resize) contam só páginas mapeadas. Regra: função de mapeamento que "pode falhar" retorna resultado; quem avança limite/contador usa o retorno.
- **OTA exige assinatura Ed25519 da release key (SESSION_258, auditoria #7):** sha256-only era self-consistent (MITM forja o hash do próprio blob). Fix: o servidor assina o DIGEST sha256 no manifest (`sig`, 64B hex), o kernel verifica contra `TRUSTED_PUBLIC_KEYS` via `identity::verify_update_signature` (só trusted — NUNCA session pk) e rejeita `unsigned_update`/`bad_signature` antes de gravar slot. Tooling: `tools/ota_sign.py` (Ed25519 puro RFC 8032, zero deps, selftest contra vetor 1 — interopera com ed25519_compact). Chave: PUBKEY pinada no kernel, SEED só no host do dono (`target/ota_release_seed.hex`, gitignored; rotação = novo par + nova entrada no pin). Regra: update sem assinatura não entra no slot.
