...
# Current Sprint: **v1.9.99 TEST** (SESSION_275) — Jarbas compositor honesto J-01–J-09 (click/chat/toast/dock + HDA único). SESSION_274 GPU compute honesto + MHI tier0 CE wired intacta. Base AIOS-First (ADR-0088): DeviceTree+plano k_ai (s271), storage+Trust+HITL (s272), Trinity único + HUD/HITL (s273). Mesh graph UI (s261) + ADR-0081 intactos.
...
# ADR-0057 Compute Dispatch SMP+GPU+NPU: WS-A wake multi-AP (SIPI direcionado sequencial + stack/PerCpu por-AP + retry; `-smp 4`→APs=3, CorePools r0=1 r1=2 r2=1; contador unificado; bin::smp emagrecido); WS-B/C `cortex::compute` dispatcher (gated `ap_pollable`, deadlock-proof); WS-D **canário Ready ≠ matmul no device** (SESSION_274: `gpu_matmul`/`nvidia_matmul` = `None` até KernelPack W2A8; CE = cópia Dram→Vram, não BitNet); WS-E NPU XDNA/Intel detecção+fallback software; WS-G #412 `cortex::decode` (self-test PASS). On-demand AP-worker (IDT/IPI) + GPU W2A8 + driver NPU = Layer S/HW.
...
- **Desenho fora do `render()` do compositor é apagado (SESSION_261/275):** `JarbasDesktop::render()` é o ÚNICO pintor de frame e o único que chama `fb.swap()` — ele apaga o back buffer e só swap() dentro dele. Draw no `tick()` **no mesmo tick** que `render()` ainda apaga (fill do back buffer no início do render). Overlay/toast/EMBED precisam de static (`MESH_GRAPH`, `EMBED_MARKS`, `RENDER_OVERLAYS`, NotificationQueue) + paint *dentro* de `render()`. Cards de mesh s242 nunca apareceram na tela pelo mesmo motivo. Dock e toasts seguem a mesma regra (J-03/J-09). Regra visual: orb (nível "um eu") pode virar o nó central de um grafo (nível "sistema") — substituir nunca, centralizar sempre.
- **Janela em `windows[]` ≠ na tela (SESSION_275):** `render()` itera floating/tiling da workspace, não o vec `windows[]` solto. Chat/apps exigem `add_window_floating` (`show_app`/`toggle_app`/`ensure_hermes_overlay`). Sem isso a janela existe, o clique/log funcionam, a tela não.
- **Hit-test com `return` cedo mata `card_click`/dock/drag (SESSION_275):** painel 35% Hermes (já removido s261) + returns antecipados engoliam o restante. Canónico: dock → cards → janelas tiled/floating; orb/mesh = miss. `CARD_ACTION` só dispara se o botão é testado.
- **Dois `init_hda` = poll no controller errado (SESSION_275):** IDT 0x30 = `k_nano::audio::hda`. k_hal Oneshot que resetava GCTL no 2º init deixava IRQ num BAR e poll noutro. Facade: `is_ready()` → skip; senão `k_nano::init_hda()`. Residual SD1 pin OUT/DAC = AWAITING_HW.
...
- **Plano k_ai ANTES dos drivers (SESSION_271/272, ADR-0088):** `k_hal::init` tardio + E1000-first + SelfHeal USB=inventário vazio. Fix: H1 idempotente pós-PCI; `boot_bind`/`boot_observe` rankeia NIC observada; SESSION_272: `StorageKind` NVMe>AHCI>USB>ATA, Escalate bloqueia Auto, Trust `(1,boot_observe,plan)`, SLIP=DEGRADED+I5. Bin só executa o plano — não decide sozinho.
- **Dois TRINITY / LCG fingindo MoE (SESSION_273):** static no bin + hermes vazio; LCG seed=42 no boot; HUD NET=HwReal e HEALTH I5 virava chat LLM. Fonte única `cortex::trinity::TRINITY`; `router_trained`; `runtime_observe` (I5/HITL ≠ LLM); HUD `net_hud_label` pintado no `render()` (lição s261).
- **GPU Ready tem três verdades (SESSION_274/275):** BAR/canário (bring-up) ≠ engine de cópia (CE) ≠ matemática no device (KernelPack W2A8). `BackendState::Ready` **não** é matmul GPU — J-05 log FE: "canary Ready / copy-engine; device math = None until KernelPack". `nvidia_matmul`/`gpu_matmul` faziam handshake DMA + `a.matmul(b)` na CPU com `drain(true)` — telemetria mentirosa. Fix s274: vendor matmul → `None`; `boot_report.gpu_ok` via `note_gpu`; `mhi_tier0_copy` só com canário CE golden + rollback CoW. cuda-oxide/kaio geram PTX no HOST; nada roda compute GPU em `no_std`.
