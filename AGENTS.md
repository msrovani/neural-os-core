...
# Current Sprint: **v1.9.99-s288 TEST** (SESSION_288 / Trinity 7 kind + Falcon3-7B) — router LOADED 25KB 93.5% + PRO.v6 1.86GB + auto_arena 50% RAM. s287 HEAP_BUDGET cap ✅; s286 emagrecer C1-C5 ✅; s282 backlog custo×anel T-001–T-075 em paralelo (T-017 metal K23).
...
- **grow_bump_auto sem budget cap = OOM (SESSION_287):** `grow_bump_auto` (allocator.rs:76) cresce o bump heap indefinidamente (256MB/passo) quando `HEAP_LIMIT` nao tem teto. Em QEMU 6G/4cores, o heap cresceu 512->2048->2304->2453MB ate esgotar o frame allocator -> OOM crash antes do AgentFleet. Fix: `HEAP_BUDGET_MB` static (default 1536) + `set_heap_budget_mb()` chamado de main.rs com `min(75% RAM, 1536)`. grow_bump_auto retorna false quando `current_limit >= budget_bytes`. Resultado: heap para em 768MB (suficiente para boot), OOM eliminado. **O budget SO controla o bump allocator** -- o `resize_heap` do TALC (usado pelo cortex para LLM load) segue seu proprio caminho e NAO respeita este cap.
...
- **ATA TCG skip e intencional (boot_observe.rs:60):** sob `-accel tcg`, o boot_observe.rs pula ATA do probe plan (`if tcg && st == StorageKind::Ata`) para evitar lentidao extrema de PIO no emulador. WHPX inclui ATA mas FAT walk de 3GB e lento (328 linhas vs 1093 TCG). **Solucao:** image mini (`--mini`) ou QEMU loader para modelos.
...
- **Falcon3-7B e 3B têm MESMA arquitetura (Fase 1 autonomia):** Configs do HuggingFace revelam que Falcon3-7B-1.58bit e 3B-1.58bit compartilham hidden=3072, 28 layers, 12 heads, kv=4, intermediate=23040 — EXATAMENTE iguais. O "7B" é apenas nome; o treino pode ser mais refinado mas os parâmetros são idênticos. O único modelo realmente maior é o 10B (40 layers). Converter Falcon3-7B: `python tools/convert_falcon3_bitnet.py --hf-repo tiiuae/Falcon3-7B-Instruct-1.58bit --output target1/PRO.v6`.
- **Hardcoded em cortex.rs era silenciosamente errado:** `FALCON3_NUM_LAYERS=22` (real=28), `FALCON3_FILE_SIZE=1.03GB` (real=1.74GB), `FALCON3_EMBED_DIM=3072` (ok), `FALCON3_MAX_SEQ=4096` (real=32768). Fix: `ModelHeader` + `parse_model_header()` lê tudo do header v6 em runtime. `v6_file_size()` delega ao header — zero duplicação.
