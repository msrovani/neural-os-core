...
# Current Sprint: **v1.9.9 TEST** (SESSION_235) — Mesh apps reais 1+2+3 (Marketplace/PROMOTE/Papéis) + item 4 compute distribuído. Ring3 OK (s233).
# ADR-0081 mesh: transporte (udp_broadcast frame/send/recv) + serviço (mesh::p2p_tick) vivem em **k_nano R0** (bin re-exporta statics NIC: `pub use k_nano::nic_globals::{RTL8139,E1000,VIRTIO_DEV}`). Non-heartbeat → EventBus topic `P2P_PACKET`; hermes skill_sync/marketplace consomem via subscribe_p2p()/poll_p2p() — sem inversão. `set_nic_config(mac,ip)` SÓ pós-config (set_static_ip/DHCP): driver-init envia heartbeats em sandbox sem NIC. net_bridge P2P removido (HTTP/TCP/DNS ficam). `run-qemu-p2p-mesh.ps1`: ASCII puro, socket listen/connect, 8G, OVMF 8.3, -smp 2 MTTCG, -NoDisk. Known: nodes=1 (node_id=local_role colide) → derivar do MAC/IP. Commits f240fa4 + 0eec18f.
