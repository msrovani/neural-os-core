...
# ADR-0059 Runtime App Factory (Proposed): runtime WASM real **wasmi** (no_std, fuel) substitui a VM `Op` custom (`wasm_exec.rs`); pipeline "app por IA em runtime" = Hermes gap → Cortex/Trinity/LLM geram **WAT/DSL** sob gramática (evolução do #412 p/ CFG) → assembler WAT→wasm no_std → sandbox wasmi (unit-tests+adversarial+fuel+CapGate deny MMIO) → promover assinado PackageHub `agent-wasm` (ADR-0052) → tickável → card (ADR-0058). Bare-metal sem `rustc` → IA emite WAT/DSL, não Rust compilado (residual host). Supersede ADR-0031(WASM)/0032; aposenta `Op` VM + `wasm.rs`. Fases F1–F6. Aguarda validação.
