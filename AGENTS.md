...
- **Falcon3 lab tok/s + OOM UI (SESSION_349):** script canónico `tools/measure-falcon3-toks.ps1 -Accel whpx`. WHPX ≈0.15 tok/s — ler `milli=`/`us=`, não `decode_tok/s` inteiro. FAT size pode ser 7B mal-nomeado → **sempre** `v6_file_size` do loader; parse in-place (sem leak 2×). Header `max_seq=32768` (Instruct denso) ≠ ctx 1.58bit (4096) → clamp no loader + `embed_for_kv` ctx≤512. `Tensor::new` sem `checked_mul` wrapa em release → OOM com `size` primo (~4.4GB) stamp `cortex_llm`.
