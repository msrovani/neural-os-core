...
- **QEMU não tem codec HDA — áudio é metal-only (SESSION_346):** o PCI HDA (`8086:2668`) existe, mas o boot loga `home=k_nano::audio::hda profile=qemu | No codecs found (degraded expected — aceite=HW)` → `init_hda` falha, SD0/SD1 nunca armam e `poll_hda_audio` retorna cedo. Nem o bug nem o fix do formato/BDL são observáveis no QEMU: **não caçar bug de áudio ali**; o aceite da captura é `AWAITING_HW`.
