...
- **`virtio-drivers` crate (rcore-os) funciona com transporte modern PCI (SESSION_343):** O crate `virtio-drivers 0.13` fornece drivers maduros (net/blk/gpu/console/input) mas exige transporte **modern** (BAR-based common config, não legacy I/O ports). QEMU default para `virtio-net-pci` é modern (sem `disable-modern=on`). Requer implementar `Hal` (DMA alloc, MMIO mapping) e `ConfigurationAccess` (PCI config bridge). O `zerocopy` (dependência) compila no toolchain atual. Os drivers legacy em `virtio_net.rs`/`virtio_blk.rs` permanecem como fallback. Funções unificadas `virtio_send()`/`virtio_recv()` checam modern primeiro, depois legacy. GPU 2D via crate é viável (`VirtIOGpu` com `resource_create_2d`/`flush`) mas ainda pendente.
