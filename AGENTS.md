...
- **Sync n-sgdb:** Sempre que registrar ADR, SESSION ou IDEA_BANK, também registrar no n-sgdb (neural-sgdb MCP). Padrão: ADR → 1 memória compacta por ADR (entities `adr-XXXX` + `mom/decision`); SESSION → lote por era com one-liners do SESSION_INDEX (entities `session-XXX` + `mom/fact`); IDEA_BANK → lote por tema com statuses (entities `idea-XXX` + `mom/fact`). Sempre `scope=project/neural-os-core`; fim do lote `curate(op=commit_run)`. AGENTS.md → re-upload em mudanças materiais (entities `agents-md` + `mom/constraint`).
...
# s389 Logging QEMU+HW ADR-0092 honesty (H1–L3);
# s388 ModelHub+Trinity honesty (H1–L3);
# s387 Falcon3 LLM 1.58bit honesty (H1–L5);
...
# s382 mouse USB tablet (#542 deferred HID);
# s381 jarbas honesty (avatar/slog/barge-in);
# s383 CapGate DynSkill #600 + MicFrameRing #562;
# s382 mouse USB tablet (#542);
# s380 hermes residuals (#604 aios_net/fs + orphans);
...
# s375 event-bus honesty (bounded/Ok(n)/Latent recv);
# s374 k_ai honesty (recovery/I3/Trust/budget/Trainer);
# s373 k_hal ring/iwl/BB_END/matmul; s372 k_nano i225/nvme/slog; s371 cortex TQ2_0 ggml;
# s370 agent-core; s369 Boot Trilha A; s368 Fechamento 1.x B1/B3.1; s367 LLM response gate;
# s366 ADR-0106; s365 typed_sites; s364 Decision contract; s363 UI liveness; s362 router encode;
...
# s355 k_hal R1 honesty (Intel rings + WiFi scan);
# s354 NSGDB 1.1.20 + K33 Tickv GC + k_nano timeout honesty;
# s353 bughunt heap/infer; s328 Full Infer D+B+C;
...
# PRIORIDADE lab Falcon3-3B 1.58 tok/s (WHPX loader-only — sucesso medido):
#   tools\measure-falcon3-toks.ps1 -Accel whpx -RamGB 6 -Smp 4
```

Config: WHPX + Haswell/TCG, e1000 + `-netdev user`, VirtIO-GPU. SLIP/COM2 frozen.
Disk: `if=ide`. Ver `run-qemu-whpx.ps1` + `SESSION_149`/`150`. Lab tok/s: `HOWTO.md` § PRIORIDADE + SGDB `scope=neural-os-core/dev`.
...
- **Dedup com nonce auto-incrementado não dedupa (SESSION_410):** mesh_knowledge deduplicava por payload que incluía `clock.tick()` a cada TX — o payload nunca repetia, o dedup nunca derrubava, e a memória replicada (RX MEM 648×) cresceu ~11KB/tick até OOM nos workers de 1G. Dedupe válido = fingerprint de CONTEÚDO (FNV-1a TX+RX). Corolários: (1) toda estrutura de "aprendizado de runtime" (observations, approvals, marketplace, knowledge) precisa CAP + evicção FIFO — sem cap, OOM a médio prazo no Master que vive mais; (2) escalada de saúde para LLM (`should_escalate_health_to_llm`) com `sched=Violation/Warning` vira loop de feedback (Master gerou 233 intents "diagnostique e corrija" = 7× workers) — escalada é para saúde NOVA, não o eco da própria decisão; (3) I3 judge antes do dado = violação fantasma — invariante de "nada aconteceu" só vale depois do fluxo que produz o dado (push pattern `note_trust_entries`).
- **Loop por tecnologia (§1–§13 TECNOLOGIAS): medidas derivam — nunca copie números de doc (SESSION_392–401):** §13 "116 blobs/~12.5MB" era cache stale; medido real = **90 blobs / ~14.4MB**. Regra SESSION_329 reforçada: métricas de doc = `update_tecnologias.py` (criado em s401); re-medir antes de escrever. Onda 6 ficou **recon-only** na 1ª passada — revisão pegou e aplicou (s401).
- **ACHI bit ATAPI=1 em disco ATA + PRDT única = fail-aberto DMA (s397):** `CFL=0x25` seta bit A (ATAPI) em FIS ATA; QEMU ignora, HW real falsa. PRDT única em buffer >4MB/não-contíguo falso-positivo; cache de command table elimina leak. Padrão: `const fn` p/ encoding MI Intel validado contra `intel_gpu_commands.h` — `MI_BATCH_BUFFER_START`/`MI_FLUSH_DW`/`PIPELINE_SELECT` estavam com hex errado (s392 info).
- **`StackSizeResponse` sem address = fantasma (S254 consolidado em s392):** derivar via RSP; protocolo Limine nunca expõe endereço.
- **`cargo clean -p neural-kernel` pode reportar "Removed 0 files" (S346 confirmado):** rebuild real ~17s vs cache 0.36s — sempre `touch` main.rs para forçar rebuild real se o tempo parecer rápido demais.
- **TCP smoltcp clock tick≠ms (S252):** `now.saturating_mul(55)`; Qualquer timer de rede com tick puro é bug latente.
- **Intel ring MMIO: hex a mais + TAIL≠START + bytes≠dwords (SESSION_355):** RCS estava em `0x120000` (mesma classe BCS `0x220000`→`0x22000`); TAIL escrito no slot de START; `RING_CTL=4096` em vez de `0x3001`; `tail` em dwords vs HEAD/TAIL Gen9 em **bytes** com wrap — `wait_idle` nunca batia de verdade. Conferir i915 `RING_*` offsets antes de tocar engine. **iwlwifi scan fabricar SSID** (JARVIS-NET) = honesty falsa pior que timeout.
...
- **Dataset sintético que não é fala (SESSION_346):** `train_stt.py::synthesize_pcm` gerava 3 senoides com `f0/f1/f2 = f(ord(ch)-ord('a'))` — o CTC aprendeu "bipes por letra". Nenhuma mexida em arquitetura/kernel conserta isso; **validar que o dado de treino é a modalidade real** antes de otimizar o modelo. Validação em `CORPUS[:8]` (subconjunto do treino) dava acurácia sem significado: holdout tem que ser **disjunto**.
- **Barge-in que só limpa o ring não interrompe (SESSION_346):** o buffer `stream_tts` do `JarbasAgent` redrenava no tick seguinte. Interrupção precisa de **generation id** (`TTS_GENERATION`) consultado pelo produtor. E "pular o TTS completo" com um `bool` **perde a frase final** (a que nunca fechou durante o generate): guardar o ledger do que já foi falado e falar só o restante.
- **`cargo clean -p neural-kernel` pode reportar "Removed 0 files" + check cacheado em 0,15 s (SESSION_346):** o rebuild real mostra o tempo verdadeiro (~17 s). `touch crates/neural-kernel/src/main.rs` força a recompilação de verdade quando o check volta rápido demais.
- **Teste flaky em `cargo test --workspace` (SESSION_346):** `k_ai::boot_observe::ingest_bootlog_cross_boot_order_and_cap` falha no run paralelo e passa isolado/em `-p k_ai --lib` (45/45) — statics compartilhados entre testes. Antes de tratar como regressão, rodar a suíte do crate sozinha.
- **QEMU EMULA HDA — "No codecs found" era o nosso driver (SESSION_346):** `run-qemu-whpx.ps1` sobe com `-audiodev dsound` + codec `hda-duplex` (vendor `0x1af40022`) e o boot loga `Found HDA: 8086:2668`. A conclusão antiga ("QEMU intel-hda mudo, aceite=HW") era sintoma de 4 bugs do driver: GCTL.CRST invertido, ICW sem kick (BUSY em ICS), verbo gravado em [11:0] em vez de [19:8], e enumeração parando no root. Corrigidos, `init_hda` passa a armar SD0+SD1 **no QEMU** — o aceite de bring-up é `capture path ready CAD 0` + `SD0 capture: BDL @ … size=64KB` no log, não `AWAITING_HW`.
- **HDA enumera em DOIS níveis e o AFG não tem widget cap (SESSION_346):** NID 0 → NODE_COUNT = `0x00010001` → só o Audio Function Group (nid 1); o tipo do AFG vem de `FUNCTION_TYPE` (0x05), porque `AUDIO_WIDGET_CAP` nele responde 0 (QEMU `duplex_params_audio_func_` não declara o param) e a enumeração morre com `mic_pin=0 adc=0`. Os widgets (DAC=0x0 / ADC=0x1 / pin=0x4) vêm do NODE_COUNT **do AFG** (`0x00020004` → nids 2..5). O pin de entrada escolhe-se por Default Device: `0xA`=Mic In, `0x8`=Line In (o `0x4` que estava lá é S/PDIF **Out**).
- **IDs de parâmetro HDA são tabela, não analogia (SESSION_346):** `PIN_CAP=0x0C`, `PCM Size/Rates=0x0A`, `CONNLIST_LEN=0x0E`, `AMP_IN=0x0D`, `AMP_OUT=0x12`, `VOL_KNB=0x13` (HDA §7.3.4 == `AC_PAR_*` de `hw/audio/intel-hda-defs.h`). Pedir o ID errado **não falha**: o codec responde 0, o que vira "pin sem capacidade" — `PARAM_PCAP=0x0A` foi o que manteve `spk=0 mic_pin=0` mesmo com a enumeração em dois níveis já correta.
- **Stall do boot 8c é em `K33[28] sgdb` (SESSION_346):** os logs antes e depois do fix de HDA param na MESMA linha (480→492 linhas), PHASE 0–5 `status=ok`, PHASE 6/7 nunca chegam. Não é regressão de áudio — é a frente de boot (pins/FAT/tickv): enquanto durar, `VOICE_STATE` e os contadores de captura não têm onde aparecer.
- **Playback drenado por tick ≠ pelo clock do áudio (SESSION_352):** `AudioMixerAgent` puxa 1024 amostras/tick e `write_hda_playback` descarta o que não cabe no anel DMA (pacing por LPIB; buffer = 1024 frames × 16 entradas = **341 ms**). A 60 Hz o mixer puxa 61 440/s contra 16 000/s do dispositivo → rajada > 341 ms perde até ~74% da fala em `PLAY_SAMPLES_DROPPED`; a 120 Hz ~87%; abaixo de ~15,6 Hz vira starvation. Como o ADR-0104 deixou o tick adaptativo (30/60/120), a cadência da UI muda a fidelidade do áudio: puxar `min(free_frames, …)` (ou Δt × `VOICE_RATE`), nunca bloco fixo.
- **Auto-barge-in sem AEC (SESSION_352):** VAD `start` com `PLAYBACK_RING.available() > 0` chama `request_interrupt()`, que limpa a fala **e** `cancel_active()` na inferência. Não há AEC no repositório → em HW real o assistente se interrompe e destrói a própria resposta (o piso adaptativo do VAD converge para o próprio eco). Interromper fala ≠ abortar raciocínio: gate de duplexidade explícito antes de cancelar inferência.
- **`STT_UNCERTAIN` sem consumidor (SESSION_352):** o tópico é publicado (`stt.rs:769`) e ninguém escuta → incerteza de transcrição = **silêncio** para quem fala; a honestidade de não inventar texto existe na origem mas não chega ao usuário. Tópico novo só está "feito" quando tem consumidor.
- **Áudio de 50 Hz no EventBus é topologia errada (SESSION_352):** `publish()` clona o payload **por assinante** em `VecDeque` **ilimitado** — `AUDIO_FRAME` (640 B a ~50/s, ≥2 assinantes) = ≥100 alocações/s e vazamento sem limite se um consumidor parar de drenar (agente rebaixado a EventDriven). Stream de alta taxa vai em ring SPSC de consumidor único; o barramento fica com controle. Idem: `pcm_buffer` da sessão de voz cresce sem teto (só sai no VAD end) e `begin_job` aloca `logits = n_frames × vocab` (768 KB p/ 30 s) — greedy CTC só precisa da emissão anterior.
- **Wake-word: janela e features (SESSION_352):** o MLP lê `energy_history[..16]` = os 16 frames mais **VELHOS** de uma janela de 64 (~1,28 s) e só é avaliado quando ela enche → latência de detecção até 1,28 s, sem nunca ver o "JARBAS" recém-dito; a entrada é RMS pura (sem espectro), então o detector de fato é o padrão de 2 picos. A acurácia "98,4%" do comentário não tem script nem dataset no repo — **claim sem artefato = overclaim** (mesma classe do corpus sintético de STT).
- **Provar não-regressão comparando com o log ANTERIOR à mudança (SESSION_346):** o boot 8c parou em `TTS boot greeting 168160 frames` (pós PHASE 5, sem PHASE 6/7) — o log da árvore antes da mudança para na MESMA linha (527 vs 523 linhas). Mesmo ponto de parada com código diferente ⇒ pré-existente. Regenerar a imagem e re-testar é obrigatório antes de culpar/defender um fix de outra sessão (o `STATE.md` do s345 dizia "FIXED código" e o stall persiste).
- **`tools/run-qemu-8c-ui-loop.ps1` não parseia (SESSION_346):** `Parser::ParseFile` → "missing closing brace" (linha 55, col 59), erro em cascata (as chaves balanceiam no tokenizer → não é o `}` literal). Para rodar 8c, chamar `run-qemu-whpx.ps1 -Smp 8 -RamGB 4` direto; ele BLOQUEIA, então destaque com `Start-Process` (o tool de background não existe aqui). Host com 4,8 GB livres → `-RamGB 4` (6 GB estoura).
- **Sessão numerada com colisão (SESSION_346):** `STATE.md`/`SESSION_INDEX.md` estavam sujos por outra frente (s345, FAT pins) na mesma árvore — não sobrescrever: registrar com o próximo número livre (346) e deixar os arquivos do outro dono intactos (reforça SESSION_264/277).
- **BGE.BIN lookup_file_size hangs ANTES do sz guard (SESSION_340 — CORRIGIDO):** o `sz > 8MB skip ATA PIO` em main.rs:2848 nunca era alcançado — `lookup_file_size("BGE.BIN")` travava na caminhada da root dir cluster chain via ATA PIO. Fix: `ROOT_DIR_CACHE` (256KB static) popula a chain uma vez; subsequentes usam cache. TSC timeout 2s previne hang. ATA BGE block simplificado de 40→15 LOC via `read_file_from_dev`. 11 chamadas → 1 leitura ATA + 10 cache-hit.
- **Heap-wrap 2⁶⁴: budget em MB-de-RAM não previne o wrap (SESSION_339):** a janela endereçável do bump heap é **~2 GB** (`.kheap` no FIM da imagem; `heap_start + offset` cruza 2⁶⁴ em ~2044 MB) — o budget (5376 MB) capava em MB-de-RAM, não em offset. Fix: `BUMP_MAX_OFFSET = usize::MAX - heap_start - 4096` + recusar honestamente **ANTES de mapear** (`HEAP fail refuse need=XMB window=~2034MB (agente=<nome>)`). E **"Heap:128MB" era métrica mentirosa** (MemoryAgent hardcodava 128 no branch sem-modelo; o real era o piso 512 MB) — 3 números de "heap" inconsistentes = a armadilha de debug que fez o 1º fix (HEAP_EXT_BASE) falhar. **Allocs grandes bypassam o grow** (TALC direto → OOM handler sem passar por `grow_bump_auto`) — instrumentar o **OOM handler** com o nome do agente (`tick_in_progress()`, lock-free) + canal FB.
- **Intrinsics SSE2 (`_mm_loadu_si128`/`_mm_storeu_si128`) sob `#[target_feature]` são MAL-COMPILADOS no target soft-float (SESSION_336):** LLVM baixa para scalar que **zera o byte 1 de cada u32** (canal G no BGRX) — o orb renderizava azul-escuro e o frame cost subia 9× (159 ms). Host tests NÃO pegam (host tem SSE2 nativo). Regra: **não usar intrinsics XMM no kernel** (`x86_64-unknown-none` soft-float); usar `movnti`/`_mm_stream_si64` (operando GP) ou `copy_nonoverlapping` (memcpy do compilador). Reforça SESSION_249b.
- **Fluidez de UI = medir o frame cost vs o alvo ANTES de otimizar o desenho (SESSION_331/334):** o orb estava capado a 15 fps por LCM de gates (s331) e depois a 30 fps por `TARGET_FPS=30` (const), **não** por compute (bench 112-130 µs/frame vs 33 ms de budget). Antes de "otimizar pixels", comparar o custo medido com o alvo — o limite costuma ser o gate/o alvo. Cadência de UI = wall-clock.
- **Damage rects > full swap (SESSION_334):** um booleano `need_full` copia 3.6 MB (1280×720×4) por 1 px mudado. `DamageList` heapless (`[DamageRect;16]`, merge/clip/overflow-merge, zero alloc) + cada camada pintada empurra seu rect (velho+novo); full só na 1ª frame/vcon/power. (Wayland/X11 DAMAGE/Android/DXGI dirty rects; TouchGFX cacheable container p/ chrome estático.)
- **FB Write-Combining vs Uncached + `movntdq` não compila no soft-float (SESSION_335):** UC serializa cada store; WC coalesce (Linux `efifb`/`pgprot_writecombine`) — exige `IA32_PAT` entry 4=WC + PTE PAT bit (back buffer fica **WB**, é RMW). E `_mm_stream_si128`/`movntdq` (xmm) **não compila** no `x86_64-unknown-none` soft-float (`LLVM: Do not know how to split this operator's operand!`): usar `movnti`/`_mm_stream_si64` (operando GP) + `sfence`. Ganho é **metal-only** — QEMU mascara.
- **Aceleração de HW: sempre gated + default-OFF + fallback (SESSION_335):** cursor em plano de HW (`CURCNTR` mode 0x27 + `CURBASE`=offset GGTT 32-bit — não PA) e blit BCS (src/dst **pinados na GGTT**, offsets, não ptr/PA cru) só ativam com canário/readback OK; QEMU/não-proven cai em software/CPU. Nunca default em HW (tela preta é irrecuperável sem reboot). `page_flip_hw` (disable/enable do plano) é unsafe — não usar.
- **Gate por LCM acidental capa fps — o renderer não era o gargalo (SESSION_331):** o orb estava capado a **15 fps a 60 Hz** por dois gates de tick independentes (`tick%3` do orb × `tick < hz/30` do frame) que batiam a cada 4 ticks. Simular a aritmética dos gates ANTES de culpar o desenho. Cadência de UI = **wall-clock** (`now_us - last >= 1e6/TARGET_FPS`), nunca contagem de frames.
- **Animação por contagem de frames ≠ por tempo (SESSION_331):** `spin += 3.2`/frame, `sinf(frame*0.028)`, partículas `frame*speed` → mudar o rail muda o tempo e o drop de frame atrasa a animação. Acumular fase com **dt do TSC**; LUTs inteiros (`SIN_LUT`/`sin_q8`), zero `sinf/sqrtf`/divisão por pixel no render path.
- **Instrumento invisível = instrumento inexistente (SESSION_330/331):** `slog` **não pinta o FB** (só serial, ausente no metal, + `boot_ramlog` em phys `0x1000_0000` não reservado no PMM → clobberable). Diagnóstico de metal DEVE usar canal FB (`boot_progress_line`/`console_print`/painel); toda instrumentação USB do `3bcb49ed` era `slog` → nada apareceu.
- **Política fora da UI — "tudo é agente" também vale para a tela (SESSION_331):** o painel Hub Health NÃO decide; um `HubHealthAgent` (EventDriven) decide abrir/fechar/pill/worst e publica `HUB_HEALTH_STATE`; o compositor só desenha o snapshot. Dados **medidos**, e **`n/a` ≠ 0** (painel de diagnóstico que mente derrota o propósito).
- **FFT no caminho de áudio é assassino (SESSION_331):** DFT ingênua com `sinf/cosf` por amostra = ~129k transcendentais por buffer (mixer/voice) → stall de scheduler no metal. Fix: Goertzel (2 mul + 2 add/amostra/bin) + janela Hann em LUT (`spin::Once`) + **tap capado** (1024/256) — o VAD/STT continua vendo todos os samples.
- **Paleta/estado por string `&str` = morto silencioso (SESSION_331):** `jarbas_color` casava `"DREAM|THINK|IDLE|ALERT"` mas o writer emitia `"SPEAK|PROCESS|LISTEN|IDLE"` → Speaking/Listening/Thinking nunca acendiam. Usar **enum**, não match de string.
- **Métricas/versões de doc derivam — medir o repo, não copiar (SESSION_329):** AGENTS/README/TECNOLOGIAS/SUMMARY/ROADMAP/codemap repetiam "~26K LOC / 180+ arquivos / ~50 agentes / v1.9.5"; o real medido é **~148K LOC / ~671 `.rs` / 41 nativos / v1.9.99-s328**. Números de doc são *cache*; a fonte é `Cargo.toml` (members) + contagem de `.rs`/LOC + `skills/agents/*/SKILL.md` + `cargo test --no-fail-fast`. Medir antes de escrever.
- **Falcon3-3B shapes canônicos (SESSION_329):** **22 layers / hidden 3072 / 12 heads / 4 KV heads / intermediate 9216 / vocab 131072** (`tools/convert_falcon3_to_v6.py`). README dizia 30/2560 — errado. Falcon-Edge-3B ≠ Falcon3-3B.
- **Falcon3 lab tok/s + OOM UI (SESSION_349/351/353):** script canónico `tools/measure-falcon3-toks.ps1 -Accel whpx`. WHPX ≈0.15–0.16 tok/s — ler `milli=`/`us=`, não `decode_tok/s` inteiro. FAT size pode ser 7B mal-nomeado → **sempre** `v6_file_size` do loader; parse in-place (sem leak 2×). Header `max_seq=32768` (Instruct denso) ≠ ctx 1.58bit (4096) → clamp no loader + ctx≤512. `Tensor::new` checked_mul **não basta** — `vec![0.0; a*b]` ainda OOM; usar `f32_zeros`/`try_reserve` + **`is_valid()`** (shape cheio + data vazia = OOB). Cap GlobalAlloc 256 MiB = falso positivo no load (~315 MB); refuse só se size > janela bump (~2 GB). **s353:** `forward_with_kv*` → `apply_one_layer` (fonte única); mask nunca `(1,1)`; soft_stride pad KV; ternary dispatch valida `is_valid`; testes `bughunt_s353` 12/12. WHPX saudação T+800 ainda AWAITING re-teste. HW: `PACK_LLM=all python tools/build_image.py --hw --unified --build-boot --size 8192` → `target/usb_hw.img`.
- **Attn/FFN duplicada = bug latente (SESSION_353):** `apply_one_layer` endurecido + `forward_with_kv` ainda com unwrap/`up.data[i]` = duas verdades. Fix profundo = **delegar**, não patch paralelo. Soft-stride que skipa layer sem pad KV → `k_all` unwrap panic quando stride muda. OOM stamp fora de AGENT_TICK_BUSY exige `note_background_agent` (AP idle).
- **Heap AIOS (SESSION_350):** Observe→Plan→Act→Verify→Remember em `heap_aios` + InferQueue antes do prefill; grow refuse só atomics.
- **Test suite host — comando real (SESSION_329):** `cargo test --workspace --exclude neural-kernel --exclude boot --no-fail-fast` = **784 pass / 6 fail**; sem `--no-fail-fast` o cargo para na 1ª suíte que falha (rodam só 5 suítes). Teste de integração `cortex` exige `python tools/gen_test_gguf.py` → `target/test_tq2_0.gguf`, senão nem compila. Os 6 fails são pré-existentes (`hermes::wasm_build`/`cognitive_bridge`, `jarbas::jarvis::soul_*`).
- **INDEX de ADRs é inventário vivo (SESSION_329):** ADR nova = linha nova em `docs/architecture/INDEX.md`; a nota "lacuna 0090–0099 vazia" envelhece rápido (0090/0091/0093/0094 existiam sem entrada). Legados/`conflito_id` → `docs/archive/notes/` via `git mv` + atualizar INDEX **e** os ADRs que os citam (ex.: 0102); nunca renomear sem migração.
...
- **`build_image.py --hw --unified` sem `PACK_LLM` = timeout (sessão atual):** default é `falcon3` — baixa/converte Falcon3 3B inline (multi-GB) → hang 240s+. SEMPRE setar `PACK_LLM=2b` (ou `falcon3` se target1/ já tem o .v6) ANTES de rodar, ou usar `--mini` (`PACK_LLM=none`).
...
- **GGUF type IDs são CONTRATO ggml (`gguf-py/constants.py`), não enum livre (SESSION_309 / **corrigido SESSION_358**):** tensor types: `0=F32 1=F16 2=Q4_0 3=Q4_1 6=Q5_0 7=Q5_1 8=Q8_0 9=Q8_1 10=Q2_K … 15=Q8_K 30=BF16 34=TQ1_0 35=TQ2_0`. SESSION_309 mapeou erroneamente `2=BF16`/`25=TQ2_0` (25=I16 oficial) — quebrava Q4_0 reais. Metadata value types: `0=UINT8 … 6=FLOAT32 7=BOOL 8=STRING 9=ARRAY 10=FLOAT64`. Gate: validar loader contra GGUF REAL + `tools/gen_test_gguf.py` com IDs oficiais.
- **"1.58bit" no nome do Falcon3 é método de TREINAMENTO, não formato do arquivo (SESSION_309):** `Falcon3-*-1.58bit-q2b0.gguf` tem tensores BF16 (**type 30**) + F16 — GGUF ternário (TQ2_0) só em quantizações nativas ternárias. Não assumir o tipo pelo nome; ler o header.
...
- **Full Infer D+B+C (SESSION_328 / ADR-0057 WS-H):** `generate_*` no `CortexAgent::tick` congela Display sob `AGENT_TICK_BUSY`. Contrato: submit → `infer_queue` → `poll_slice` (InferWorker/AP idle) 1 token/slice; `LLM_STREAM`+`INFER_TTS_PARTIAL`; barge-in `cancel_active`. Não ligar `agent_tick_offload_safe` genérico — só InferQueue fora do lock.
- **Residuais InferQueue NÃO entram no mesmo entregável (SESSION_328 decisão):** (1) GPU/NPU W2A8 no job = WS-D/E Layer S — `dispatch_ternary` já é o ponto; sem Ready/KernelPack/FW = zero ganho; aceite UI ≠ prova aceleração. (2) Prefill AirLLM layer-yield = sprint seguinte (hiccup prompt). (3) **CPU** `bitnet_w2a8` (ADR-0084) ≠ **GPU** BitLinearW2A8 (WS-D) — não confundir.
- **Pós-s328 ordem:** aceite metal → Prefill yield → métricas/budget TSC → Layer S (não GPU antes do aceite).
...
- **Escada de instrumentos FB = bisector canônico HW real (SESSION_316):** serial é invisível no metal — cada build adiciona UM instrumento lock-free/volatile no FB: `diag_mark` (barras 1–8) → `diag_stamp_agent` (nome do agente) → `diag_stamp_exception` (bridge EXC_FB_FN) → `tick_stage` (bridge TICK_STAGE_FN) → **heartbeat `T=<hex>` no timer IRQ (y=80) = discriminador vivo-vs-morto** (T avançando = thread girando com IRQ viva; T congelado = freeze dentro de IRQ/IF=0) + dígito `S<n>` (barras são ilegíveis em foto). Registrar os bridges no `claim_graphics` (tick 1 do DisplayAgent).
- **TicketLock não é reentrante (SESSION_316):** double-lock no mesmo contexto = self-deadlock eterno (serving só avança quando o guard externo cai) — determinístico, sem exceção. IRQ que toque qualquer TicketLock enquanto uma thread segura = deadlock IRQ-vs-thread (CPU inteira congela). `#[repr(C)]` obrigatório: instrumentos leem ticket@0/serving@8 via ponteiro cru — repr(Rust) reordena e o read pega bytes do `data`.
- **lazy_static wrapper ZST (SESSION_316):** `&TRINITY` aponta pro wrapper, NÃO pro valor — read cru lia estáticas adjacentes (t=0xffffffff807f293d = VA de kernel = lixo). Usar `&*TRINITY`. **Validar o instrumento contra o que ele lê:** stamp com valor absurdo onde esperava contador = lendo struct errada (2 bugs do instrumento pegos assim).
- **smoltcp poll() gira para sempre com poll_delay()==ZERO (SESSION_316):** socket com trabalho cuja resposta nunca sai (TX degradado em HW real) + tráfego LAN real (broadcast/ARP/mDNS) = spin eterno. QEMU/slirp nunca entrega esse tráfego. Fix: cap 64 iterações + contador `POLL_LOOP_CAP` (AIOS: degrada, não congela).
- **xHCI HCRST pode deixar PP=0 → CCS lê 0 para sempre (SESSION_316/327):** "nenhuma porta CCS" no metal com MMIO OK = assinatura. Fix: RMW PP=1 em TODAS as portas pós-run (preservar CCS/PLS/SPEED, RW1C escritos como 1) + dump PORTSC pre/pos. Estudo Redox (lib-1): Redox NUNCA seta PP mas VERIFICA (pânica se PP=0 com CCS=1); USB3 CSC só chega com PED já setado; hub port power/reset = class requests ao hub device (usbhubd), não PORTSC; route string no slot context (bit 26 Hub + NumberPorts slot.b); TTT/MTT = TODO no Redox.
