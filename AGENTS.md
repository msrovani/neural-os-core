...
# Current Sprint: **v1.9.99 TEST** (SESSION_274) — GPU compute honesto + MHI tier0 CE wired. Base AIOS-First (ADR-0088): DeviceTree+plano k_ai antes dos drivers (s271), storage+Trust+HITL no plano (s272), Trinity único + HUD/HITL honesto (s273). Mesh graph UI (s261) + ADR-0081 intactos.
...
- **GPU Ready tem três verdades (SESSION_274):** BAR/canário (bring-up) ≠ engine de cópia (CE) ≠ matemática no device (KernelPack W2A8). `nvidia_matmul`/`gpu_matmul` faziam handshake DMA + `a.matmul(b)` na CPU com `drain(true)` — telemetria mentirosa. Fix: vendor matmul → `None`; `boot_report.gpu_ok` via `note_gpu` (não `true` hardcoded); `mhi_tier0_copy` só com canário CE golden + rollback CoW. cuda-oxide/kaio geram PTX no HOST; nada roda compute GPU em `no_std`.
