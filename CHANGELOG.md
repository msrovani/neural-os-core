...
- **Aceite QEMU 8c:** boot chega a PHASE 5 `ok` sem panic e para na MESMA linha do log anterior às mudanças (`TTS boot greeting`) ⇒ o stall do 8c é **pré-existente** (pista FAT/pins do s345), não desta mudança. QEMU **emula** HDA — a leitura `No codecs found … aceite=HW` estava errada e o bring-up foi ressuscitado no **FIX 2** abaixo (SD0+SD1 armam no QEMU). `tools/run-qemu-8c-ui-loop.ps1` não parseia (erro de chave em cascata, linha 55) — pendente.
- **FIX 2 — HDA em QEMU: o codec sempre respondeu.** Quatro bugs do driver mantinham o bring-up morto: `GCTL.CRST` invertido (ficava **em reset** e validava isso como sucesso), ICW sem *kick* (lia o próprio ICW, que é write-only), verbo gravado em `[11:0]` em vez de `[19:8]` (codec decodifica em `[19:8]`) e enumeração parando no root.
- **FIX 2 — enumeração em 2 níveis:** root → AFG via `FUNCTION_TYPE` (0x05), porque o AFG **não** declara `AUDIO_WIDGET_CAP`; widgets vêm do NODE_COUNT do AFG. Tabela de params corrigida (`PIN_CAP=0x0C`, não 0x0A; `AMP_IN=0x0D`, `AMP_OUT=0x12`, `VOL_KNB=0x13`). Pin de mic por Default Device (0xA Mic In > 0x8 Line In) — o `0x4` anterior é S/PDIF **Out**.
- **FIX 2 — aceite QEMU:** `capture path ready CAD 0` + `SD0 capture: BDL @ … size=64KB` + `SD1 playback` — `init_hda` deixa de falhar (`vendor=0x1af40022`, `mic_pin=5 adc=4 spk=3 dac=2`). `cargo check` 0 erros; `cargo test -p k-nano --lib` 192/192.
