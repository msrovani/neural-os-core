...
**Pista ativa:** **ADR-0088 operacionalizada no boot (SESSION_271–272)** — DeviceTree+plano k_ai (NIC+storage+Trust+HITL). Residual #513 / TODO #18: `measure_bandwidth` + BMIDE. ADR-0086 instalação+OTA permanece ✅.
...
**Feito recente (SESSION_272):** plano de bind completo no boot — storage NVMe>AHCI>USB>ATA no DeviceTree; Trust `(1,boot_observe,plan)`; Escalate ≠ Auto; cards PnP; HANR; SLIP degradado; Cortex sem pesos (honesto). Residual: `measure_bandwidth` / BMIDE (#513).
