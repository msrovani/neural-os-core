...
pub use jarvis::display::avatar::AvatarState;
...
impl SoulProfile {
    ...
    pub fn load_from_fat32() -> Self {
        ...
        k_nano::serial_println!("[SOUL] SOUL.MD nao encontrado. Usando perfil padrao.");
        profile
    }
}
...
pub fn persona_pipeline(text: &str, ego: &mut EgoLayer, session: &mut SessionHistory,
    ...
    // 11. Ego
    ego.learn("persona", emotion.dominant() != jarvis::jarvis::Emotion::Neutral);
    ...
    // 16. Audit
    let _ = k_nano::AUDIT_TRAIL.lock().push(tick, "pipeline", "persona", text.as_bytes());
    output
}
