...
pub fn run_benchmark() {
    ...
    crate::serial_println!("[BENCH] GPU Benchmark (TFLOPS) — matmul ternário");
    ...
    for &n in &sizes {
        ...
        let (a, b) = match (a_opt, b_opt) {
            (Some(a), Some(b)) => (a, b),
            _ => { crate::serial_println!("[BENCH] {}x{}: alloc failed", n, n); continue; }
        };

        let ops = 2.0 * (n as f64) * (n as f64) * (n as f64);

        let warm = gpu_matmul(&a, &b);
        if warm.is_none() { crate::serial_println!("[BENCH] {}x{}: matmul failed", n, n); continue; }

        let mid = crate::interrupts::TIMER_TICKS.load(Ordering::Relaxed);
        let runs = 3;
        for _ in 0..runs { let _ = gpu_matmul(&a, &b); }
        let end = crate::interrupts::TIMER_TICKS.load(Ordering::Relaxed);

        let ticks = (end - mid) as f64;
        let secs = ticks / 18.2;
        let tflops = (ops * runs as f64 / secs) / 1e12;
        crate::serial_println!("[BENCH] {}x{}: {:.3} TFLOPS ({:.1}s CPU, {} runs)", n, n, tflops, secs, runs);
    }
    crate::serial_println!("[BENCH] GPU backends: CPU fallback (v1.1.1 pipeline OK)");
}
