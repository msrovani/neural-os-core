...
impl VisionAgent {
    pub fn new() -> Self {
        VisionAgent {
            frame_receiver: k_nano::EVENT_BUS.subscribe("CAMERA_FRAME"),
            last_frame: Vec::new(),
    ...
    fn process_frame(&mut self, frame: &[u8]) -> String {
        ...
        if let Some(ref mut desk) = *jarvis::display::compositor::COMPOSITOR.lock() {
            if let Some(cam) = desk.apps.iter_mut().find(|a| a.id == jarvis::display::compositor::AppId::Camera) {
