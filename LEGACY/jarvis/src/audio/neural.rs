...
impl PocketTtsEngine {
    ...
    pub fn load(&mut self, data: &[u8]) -> bool {
        ...
        if self.loaded {
            ...
            k_nano::serial_println!("[TTS-NEURAL] Pocket TTS loaded: embed={:?}, decoder={:?}",
    ...
    pub fn generate(&self, text: &str) -> Vec<i16> {
        if !self.loaded { return jarvis::audio::tts::synthesize(text); }
        ...
        let raw = jarvis::gpu::backend::gpu_matmul(&features, &w3t).unwrap();
...
fn gelu_gpu(input: &Tensor, w: &Tensor, b: &Tensor) -> Tensor {
    let wt = w.transposed();
    let mut out = jarvis::gpu::backend::gpu_matmul(input, &wt).unwrap();
...
fn try_load_qemu(addr: u64, max_mb: usize) -> Option<alloc::vec::Vec<u8>> {
    ...
    unsafe {
        ...
        if core::ptr::read_volatile(probe) == 0xBE11BE11 {
            ...
            k_nano::serial_println!("[TTS-NEURAL] Pocket TTS encontrado em QEMU loader 0x{:x}", addr);
...
pub fn try_load_pocket_tts() -> Option<PocketTtsEngine> {
    ...
    if let Some(data) = try_load_fat("POCKETTTS.BIN") {
        ...
        if eng.load(&data) {
            k_nano::serial_println!("[TTS-NEURAL] Pocket TTS 100M loaded from FAT! GPU offload ativo");
            return Some(eng);
        }
    }
    ...
    if let Some(data) = try_load_qemu(0x100000000, 420) {
        let mut eng = PocketTtsEngine::new();
        if eng.load(&data) {
            k_nano::serial_println!("[TTS-NEURAL] Pocket TTS 100M loaded from QEMU loader! GPU offload ativo");
            return Some(eng);
        }
    }
    k_nano::serial_println!("[TTS-NEURAL] Pocket TTS ausente — formant synth ativo");
    None
}
