...
use jarvis::jarvis::{JarvisEngine, Emotion, EmotionAnalysis};
use jarvis::audio::context::build_emotional_context;
    ...
    fn tick(&mut self, tick: u64, _count: u64) -> AgentTickResult {
        ...
        while let Some(ev) = self.llm_response.try_receive() {
            ...
            if !self.greeted {
                ...
                let _ = k_nano::EVENT_BUS.publish(Event {
                    id: 0, topic: alloc::string::String::from(jarvis::audio::TOPIC_TTS_CMD),
