...
use jarvis::audio::ringbuf::AudioRingBuffer;
use jarvis::audio::vad::{VAD, VadTransition};
use jarvis::audio::ser::{extract_features, classify_emotion};
...
impl JarvisVoiceAgent {
    pub fn new() -> Self {
        JarvisVoiceAgent {
            audio_in: k_nano::EVENT_BUS.subscribe(jarvis::audio::TOPIC_AUDIO_IN),
...
impl Agent for JarvisVoiceAgent {
    ...
    fn tick(&mut self, _tick: u64, _count: u64) -> AgentTickResult {
        ...
        jarvis::audio::hda::poll_hda_audio();
        ...
        while let Some(ev) = self.audio_in.try_receive() {
            ...
            jarvis::display::avatar::process_audio_fft(pcm);
            ...
            for chunk in pcm.chunks(frame_size) {
                ...
                if transition == VadTransition::SpeechEnd && !self.pcm_buffer.is_empty() {
                    ...
                    let _ = k_nano::EVENT_BUS.publish(Event {
                        id: 0, topic: alloc::string::String::from(jarvis::audio::TOPIC_STT_TEXT),
        ...
        while let Some(ev) = self.hermes_out.try_receive() {
            ...
            let pcm = jarvis::audio::tts::synthesize(clean);

            let _ = k_nano::EVENT_BUS.publish(Event {
                id: 0, topic: alloc::string::String::from(jarvis::audio::TOPIC_AUDIO_OUT),
