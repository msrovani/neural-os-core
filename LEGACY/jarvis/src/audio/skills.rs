...
use jarvis::audio::piper::PiperEngine;
                    ...
                    if let Some(fs) = crate::fat32::Fat32Reader::new(ata, p) {
                        for name in &["PIPER.BIN", "PIPER_EN.BIN", "PIPER_PT_BR.BIN"] {
                            if let Some(data) = fs.read_file(name) {
                                ...
                                if eng.load(&data) {
                                    k_nano::serial_println!("[PIPER] Piper TTS loaded from FAT ({}): {}", if *try_slave {"slave"} else {"master"}, name);
                                    ata.slave = orig;
                                    return Some(eng);
                                }
                            }
    ...
    fn execute(&self, input: &[u8]) -> Result<Vec<u8>, &'static str> {
        ...
        let pcm = {
            ...
            match guard.as_ref() {
                ...
                _ => {
                    let audio = jarvis::audio::tts::synthesize(text);
