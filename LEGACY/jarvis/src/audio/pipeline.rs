...
use jarvis::audio::vad::VAD;
use jarvis::audio::tts::FrameProcessor;
...
impl AudioPipelineAgent {
    pub fn new() -> Self {
        AudioPipelineAgent {
            llm_receiver: k_nano::EVENT_BUS.subscribe("LLM_RESPONSE"),
            active_frame: None,
...
impl Agent for AudioPipelineAgent {
    ...
    fn tick(&mut self, _tick: u64, _count: u64) -> AgentTickResult {
        ...
        if self.frame_counter % 10 == 0 {
            ...
            let read = jarvis::audio::voice::AUDIO_RING.pop(&mut mic_samples);
            if read > 0 {
                let (_, _, _is_speech, transition) = self.vad.process_frame(&mic_samples);
                if transition == jarvis::audio::vad::VadTransition::SpeechStart {
