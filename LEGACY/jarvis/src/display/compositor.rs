...
pub fn generate_llm_icon(description: &str) -> [u8; 64] {
    let mut icon = [0u8; 64];
    if let Some(ref model) = *cortex::HWEXPERT_MODEL.lock() {
...
use spin::Mutex;
use libm::sinf;
use jarvis::display::fb::DoubleBuffer;
...
pub fn draw_text(fb: &mut DoubleBuffer, x: usize, y: usize, text: &str, scr_w: usize, r: u8, g: u8, b: u8) {
    jarvis::display::font::draw_text_scaled(fb, x, y, text, 1, scr_w, r, g, b);
}
...
pub fn render_tensor_viz(fb: &mut DoubleBuffer, x: usize, y: usize, _w: usize, _h: usize) {
    ...
    jarvis::display::font::draw_tensor_heatmap(fb, x, y, &data, 20, 20);
    draw_text(fb, x, y+85, "Attention", fb.info.width, 0,200,255);
    let bars = [0.9f32,0.7,0.5,0.8,0.3,0.6,0.4,0.7,0.2,0.5];
    jarvis::display::font::draw_attention_graph(fb, x, y+105, &bars, 100, 25);
...
impl JarvisDesktop {
    ...
    pub fn render(&mut self, tick: u64) {
        ...
        if self.avatar_visible {
            ...
            let fft_energy = jarvis::display::avatar::read_audio_energy();
        ...
        let cli_content = jarvis::display::console::get_overlay_text();
    ...
    match app.id {
        ...
        AppId::Settings => {
            ...
            if app.data.contains("[2]") || app.data.contains("sound") || app.data.contains("som") {
                let vol = jarvis::audio::settings::AUDIO_VOLUME.load(core::sync::atomic::Ordering::Relaxed);
                let clone = if jarvis::audio::settings::VOICE_CLONE_ENABLED.load(core::sync::atomic::Ordering::Relaxed) { "ON" } else { "OFF" };
                let voice = jarvis::audio::settings::CURRENT_VOICE.lock();
