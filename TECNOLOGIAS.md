...
| 1.7b | **Generative Card Desktop (UI declarativa)** | 🟡 ADR-0058 (Proposed): fundação `embedded-graphics` (`DrawTarget` sobre `DoubleBuffer`) + toolkit no_std (matrix-gui/embedded-gui/kolibri) + `UiDeclaration`/`UiRenderer`. Cards gerados por LLM (structured decode #412) ou skill WASM (RustCoder/Codex). Supersede parcial 0047-HMI. | embedded-graphics, matrix-gui, embedded-gui, kolibri | MIT/Apache | `jarbas/src/display/*`, `ui_spec.rs` | 🟡 planejado |
