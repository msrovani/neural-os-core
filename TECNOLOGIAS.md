...
| 2.10 | **K²CHJ Capability Rings MVP C+P6** | ✅ Dois AS + CR3 + SPSC + Cap + `int 0x90` + Ring3 `iretq`/stub/`ENTER_USER`. Demos non-fatal. ADR-0041 P0–P6. | Capability microkernels (seL4, Fuchsia), | MIT | `address_space.rs`, `ipc/`, `syscall.rs`, `user_mode.rs` | ✅ PoC |
...
| 2.10c | **K-IA DMA pin + Cortex mmap (P5)** | 🔄 Cap PIN/MAP_DMA + MAP_WEIGHTS; frames pinados + AS; mmap pesos eager (simulado). Vring = P8; GGUF TODO. ADR-0041 P5. | Cap + DMA pin + mmap | MIT | `k_ia_dma.rs`, `cortex_mmap.rs` | 🔄 PoC |
...
| 2.10e | **VirtIO vring + DMA pin (P8)** | 🔄 Cap VRING_SETUP; Virtqueue desc+avail+used em frames pinnados; Desc.addr=pinned phys; NIC live observe-only. ADR-0041 P8. | VirtIO 1.1 + Cap DMA | MIT | `virtio_vring.rs`, `virtio_net.rs`, `k_ia_dma.rs` | 🔄 PoC |
