...
| 1.7 | **3 Camadas Visuais: Orb + Hermes CLI + Window Manager** | 🏆 Arquitetura visual em 3 camadas Z-order com FFT audio→Orbe, overlay CLI semi-transparente, e gerenciador de janelas com mouse integrado. Tudo renderizado por software no framebuffer UEFI, sem GPU. **s294:** fill scanline + glow isqrt. **s360:** desktop **funcional** + orb cluster (cores por role mesh) + Hub Health = SystemInfo. | SmileyOS patterns, JARVIS .NET MAUI (autor) | MIT | `display/compositor.rs`, `soul_mirror.rs`, `hub_health.rs` | ✅ WHPX UI viva |
| 1.7b | **Generative Card Desktop (UI declarativa)** | ✅ ADR-0058 S1–S4: `embedded-graphics` `DrawTarget` (`FbTarget`) sobre `DoubleBuffer` + `UiDeclaration`/`UiRenderer`. Cards gerados por LLM (#412) ou skill WASM. Orb + HUD preservados; mouse close/drag/botão. **s360:** sem cards Sistema duplicados — Hub Health é o painel canônico. | embedded-graphics | MIT/Apache | `jarbas/src/display/{eg,card,compositor,agent}.rs` | ✅ QEMU desktop |
...
| 4.25 | **Mesh 6-node WHPX lab + orb cluster UI** | 🏆 Seis QEMU (3G/3c+2G/2c+4×1G/1c) + hub L2 Ethernet; STATIC 10.0.3.x; FRAG matmul Master↔Compute/Memory/Worker; orb/SoulMirror por role via `MESH_HEALTH`; Hub Health = SystemInfo. Desktop AIOS + cluster na mesma tela. | SESSION_360, ADR-0081 | MIT | `mesh.rs`, `soul_mirror.rs`, `hub_health.rs`, `tools/qemu_l2_hub.py` | ✅ WHPX lab |
