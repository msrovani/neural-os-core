...
| 1.5 | **Memory Hierarchy Index (MHI) — Alocação por IA** | 🏆 Sistema de memória em 4 tiers (Dram→Vram→Nvme→Hdd) com alocação orientada por ML. `alloc_by_tier()` + soft-migrate. SESSION_274: `register_tier0_copier` promove Dram→Vram **com dados** via CE (hook só canário golden; rollback CoW). | ZFS ARC (MFU/MRU), Hierarchical Memory papers | GPLv2 / MIT | `k_nano/mhi.rs`, `k_hal/gpu/nvidia_pascal_ce.rs` | ✅ soft-MVP + tier0 HW-gated |
...
| 2.14 | **Notifications Toast System (ADR-0062 P31)** | 🏆 Ring buffer 8 toasts → compositor overlay semi-transparente no rodapé, fade-out 120 ticks (~2s). `TOPIC_TOAST` no EventBus, `clipboard_notify::toast_push()`; DisplayAgent drena para `NotificationQueue`; **pinta só em `JarbasDesktop::render()`** (SESSION_275 J-03/J-09 — draw no tick é apagado). | EventBus patterns, Android toast | MIT | `jarbas/src/clipboard_notify.rs`, `jarbas/src/display/notifications.rs`, `jarbas/src/display/compositor.rs` | ✅ 0 err |
...
| # | Tecnologia | 🏆 Inovação | Inspiração | Licença Orig. | Arquivo | Status |
|---|-----------|------------|------------|---------------|---------|--------|
| 3.1 | **GPU Backend Universal (NVIDIA→Intel→AMD→CPU)** | 🏆 Plano `display_coex` dirige init; canário `vector_add` promove Ready; gate ADR-0047 só após golden (não PFIFO NOP). SESSION_274: Ready ≠ BitNet no device — `gpu_matmul`/`nvidia_matmul` retornam `None` até KernelPack W2A8; telemetria `drain` só conta GPU real. | nouveau, i915, amdgpu; rust-gpu (host) | GPLv2 (kernel) | `gpu/backend.rs`, `canary.rs`, `compute_abi.rs` | 🟡 fundação (honest) |
...
| 3.11 | **NVIDIA Pascal CE + MHI tier0** | 🏆 Copy engine (classe 0xc1b5) wired no `mhi_tick`: Dram→Vram com payload + rollback CoW. Hook só se canário CE golden; QEMU = metadata/AWAITING. | nouveau CE, ADR-0087 F4b | GPLv2 / MIT | `gpu/nvidia_pascal_ce.rs`, `k_nano/mhi.rs` | ✅ wiring / 🟡 evidência GTX 1050 |
| 3.12 | **cuda-oxide / kaio (PTX host)** | 📦 Geradores PTX no HOST p/ KernelPack W2A8 (#536). cuda-oxide = rustc `#[kernel]`→PTX (NVlabs); kaio = PTX zero CUDA toolkit. **Não rodam no kernel `no_std`.** | NVlabs cuda-oxide; kaio 0.5 | Apache-2.0 / MIT | pesquisa SESSION_274; alvo `tools/gpu_kernels/` | ⏳ Layer S |
...
| 4.13e | **neural-sgdb — projeto comunitário standalone (extração Modo 1)** | 🏆 Núcleo `k_ai::sgdb` extraído para repo separado `github.com/msrovani/neural-sgdb` (MIT OR Apache-2.0, zero deps, dual-mode no_std+std): Storage trait (InMemory/FileStorage), facade Sgdb, CRDT p2p, benchmarks, MCP server. OS mantém `k_ai::sgdb` AGPL interno; interop por NMD1 byte-idêntico. | ADR-0063 | MIT OR Apache-2.0 | `C:\DEV\neural-sgdb` (repo separado), `docs/api.md` | ✅ SESSION_256 |
...
**Backlog residual:** Sprint Sound ✅ (2026-07-16) — soft-float/VITS + **SD1 pin OUT/DAC AWAITING_HW** (#540). Dedup HDA k_nano↔k_hal **fechado** (SESSION_275: k_nano IRQ 0x30 fonte única; k_hal facade). Não bloqueia ADR-0042.

| # | Tecnologia | 🏆 Inovação | Inspiração | Licença Orig. | Arquivo | Status |
|---|-----------|------------|------------|---------------|---------|--------|
| 5.1 | **Intel HDA Capture + Playback** | 🏆 Driver HDA: SD0 captura + SD1 playback (BDL). CORB/RIRB, codec discovery, DMA. **Fonte única `k_nano::audio::hda` (IRQ IDT 0x30)**; `k_hal` Oneshot é facade (`is_ready()` skip GCTL). SD1 pin OUT/DAC = AWAITING_HW. | Intel HDA spec, Linux HDA driver | GPLv2 | `k_nano/src/audio/hda.rs`, `k_hal/src/audio/hda.rs` | ✅ capture / ▶️ SD1 pin |
...
| 6.7 | **MHI soft-migrate** | 🏆 `mhi_tick` metadata + DRAM memcpy seguro; registry unico k_nano. SESSION_274: DMA VRAM via CE quando hook registrado (não mais só deferido). | ZFS ARC | MIT | `k_nano/mhi.rs` | ✅ soft-MVP + CE HW-gated |
...
| 7.2 | **Modelo .bitnet (formato próprio)** | 🏆 **Formato binário proprietário para modelos ternários.** Magic "BITN", header com spec completo (hidden, layers, heads, vocab, rope, medusa), pesos compactados em 2-bit (4 pesos/byte). | GGUF (llama.cpp), safetensors (HuggingFace) | MIT / Apache 2.0 | `cortex.rs` (load_model) | ✅ 0 err · **v6 canônico (ADR-0085):** header autodescritivo (act_type/embed_type/feat), writer único `bitnet_writer.py` + paridade byte-exact, loader estrito + fallback legado, registro `cortex::model` — padroniza o pipeline K³CHJ inteiro |
