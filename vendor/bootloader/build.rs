...
fn build_uefi_bootloader() -> PathBuf {
    ...
    // neural-os: vendor patch — GOP BltOnly (Intel HD 620 etc.) → SetMode Rgb/Bgr
    let uefi_path = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap())
        .join("..")
    ...
    cmd.arg("--target-dir").arg(out_dir.join("uefi_target"));
    cmd.arg("--locked");
