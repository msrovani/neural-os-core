...
//! Bring-up: `xhci::bringup_boot_msc` (ADR-0062 P11) — stick bootável + BOOT.LOG.
...
impl UsbMassStorage {
    pub unsafe fn probe() -> Option<Self> {
        ...
        let msc_dev = match xhci::bringup_boot_msc() {
            Some(d) => d,
            None => {
                k_nano::slog_nano!("USB", "msc", "bringup_boot_msc FAIL — sem stick/enum");
                return None;
            }
        };
        ...
        if !scsi_ok {
            k_nano::slog_nano!("USB", "msc", "SCSI falhou apos bringup — MSC ignorado");
            return None;
        }
