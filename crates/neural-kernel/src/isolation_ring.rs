//! ADR-0060 — Conectores do Ring3 Isolation Ring (F6).
//!
//! **Porto seguro:** HOJE este módulo **NÃO registra** nenhum ring nativo — o
//! ring Ring3 é um BLOQUEADOR conhecido (habilitar o `iretq` real hoje causa
//! triple-fault → reboot loop; ver ADR-0060 §3/§4). Enquanto nada é registrado,
//! `hermes::app_factory::isolation_ring_available()` = `false` e os Caminhos
//! B/C (código nativo) permanecem **gated** — só o Caminho A (wasmi) executa.
//!
//! Quando o F6 for validado (ADR-0060 §6), `init_connectors()` chamará
//! `register_native_ring(ring3_run_native)` e `ring3_run_native` passará a
//! executar o blob nativo em CPL=3 isolado. Este é o **site de implementação**.
//!
...
/// Chamado no boot. **Não registra** o ring até o F6 passar o gate (ADR-0060 §6).
/// Mantém o invariante do porto seguro: nenhum código nativo não-confiável roda.
pub fn init_connectors() {
    // F6 pendente — ver ADR-0060. NÃO descomentar o register até §6 passar:
    //   hermes_crate::app_factory::register_native_ring(ring3_run_native);
    k_nano::slog_bin!(
        "ISO-RING",
        "info",
        "Ring3 isolation ring NAO pronto (ADR-0060 F6) — B/C nativo gated; wasmi (A) ativo"
    );
}

/// **Site futuro** da execução isolada de código nativo em Ring3 (ADR-0060 §7).
/// Assinatura casa com `hermes::app_factory::NativeRingFn`.
///
...
pub fn ring3_run_native(_code: &[u8], _caps: u32) -> Result<i64, &'static str> {
    Err("ADR-0060 F6: Ring3 isolation ring nao implementado (ver secoes 6/7)")
}
