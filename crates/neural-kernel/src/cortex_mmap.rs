...
pub fn demo_cortex_mmap() -> Result<(), &'static str> {
    ...
    let map = match unsafe { mmap_weights(&mut as_cortex, DEMO_WEIGHT_PAGES, Cap::MAP_WEIGHTS) }
    {
        Ok(m) => m,
        Err(e) => {
            k_nano::slog_bin!("P5", "warn", "mmap_weights: {} — Cap-only path", e);
    ...
    k_nano::slog_bin!("P5", "ok", "SUCCESS mmap pesos pages={} va={:x} phys={:x} count={}",
        map.pages,
        map.virt,
        map.phys,
        mmap_count());
