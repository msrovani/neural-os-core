...
pub fn load_bge(data: &[u8]) -> bool {
    ...
    if loaded {
        crate::serial_println!(
            "[BGE] Carregado: vocab={} hidden={} ({} MB)",
            unsafe { BGE_VOCAB },
            unsafe { BGE_HIDDEN },
            unsafe {
                BGE_WEIGHTS
                    .as_ref()
                    .map_or(0, |w| w.len() * 4 / 1024 / 1024)
            }
        );
    }
    loaded
...
pub fn bge_embed(text: &str) -> Vec<f32> {
    ...
    let Some(weights) = (unsafe { BGE_WEIGHTS.as_ref() }) else {
        return Vec::new();
    };
...
impl TeamMemory {
    ...
    pub fn recall(&self, key: &str) -> Option<&Vec<TeamMemoryEntry>> {
        self.store.get(key)
    }
