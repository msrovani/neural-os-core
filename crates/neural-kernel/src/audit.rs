//! Merkle Audit Trail — chain de auditoria com SHA-256.
//! #315.19: cada entry: tick, agent, action, payload_hash, prev_hash, entry_hash.
//! Ring buffer de 4096 entradas. Verificação de integridade.
...
/// Hash de 32 bytes via SHA-256 do tpm.rs
...
impl AuditTrail {
    ...
    pub fn push(&mut self, tick: u64, agent: &str, action: &str, payload: &[u8]) {
        let prev_hash = self.ring.last().map(|e| e.entry_hash).unwrap_or([0u8; 32]);
        ...
        let entry = AuditEntry { tick, agent: String::from(agent), action: String::from(action), payload_hash, prev_hash, entry_hash };
    ...
    pub fn status(&self) -> String {
        alloc::format!("[AUDIT] {} entradas, chain OK={}", self.count, self.verify())
    }
}
