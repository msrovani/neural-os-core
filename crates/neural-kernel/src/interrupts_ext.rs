...
fn dump_exception(name: &str, stack_frame: &InterruptStackFrame, error_code: Option<u64>) {
    ...
    putc(b'\n');
    // Diagnóstico de crash: 24 qwords do stack interrompido (endereços
    // crescentes). Valores 0xffffffff80xxxxxx = return addresses/locals
    // resolvíveis offline contra o kernel.elf (.symtab). Para no limite da
    // página para não tocar memória não-mapeada e cascatear em #DF.
