...
pub fn demo_as_r1_r3_shallow() {
    if !crate::demo_flags::RUN_CAP_DEMOS {
        k_nano::slog_bin!("AS", "warn", "skip — cap-demos off (N7 ADR-0102)");
        return;
    }
    k_nano::slog_bin!("AS", "ok", "demo shallow start (facade → k_nano::paging + k_hal::cap_gate)");
    let bar0 = match k_hal::cap_gate::hal_as_bar0() {
        ...
        _ => {
            ...
            match fallback {
                ...
                None => {
                    k_nano::slog_bin!("AS", "warn", "skip — sem BAR");
                    return;
                }
            }
        }
    ...
    k_nano::slog_bin!("AS", "ok", "MAP_BAR {:?} (expect Deny)", r3);
    if k_hal::cap_gate::check_map_bar(1, true) != k_hal::cap_gate::CapResult::Allow {
        k_nano::slog_bin!("AS", "warn", "unexpected Deny");
        return;
    }
    ...
    let mut as_r1 = match AddressSpace::clone_current() {
        Ok(a) => a,
        Err(e) => {
            k_nano::slog_bin!("AS", "fail", "clone_current fail: {}", e);
            return;
        }
    };
    ...
    match map_ok {
        Ok(()) => k_nano::slog_bin!("AS", "ok", "BAR {:#x} mapped @ {:#x} UC", bar0, AS_BAR_VA),
        Err(e) => k_nano::slog_bin!("AS", "warn", "map note: {} — continua", e),
    }
    unsafe { as_r1.activate(); }
    let touch = AS_BAR_VA as *const u32;
    let val = unsafe { core::ptr::read_volatile(touch) };
    k_nano::slog_bin!("AS", "ok", "touch BAR ok val={:#x}", val);
    unsafe { restore_cr3(kernel_l4, kernel_flags); }
    k_nano::slog_bin!("AS", "ok", "restore CR3 OK");
}
