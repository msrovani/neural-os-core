...
impl SlabBuddy {
    pub fn new() -> Self {
        SlabBuddy { slab: crate::slab::SlabAllocator::new(), heap_allocated: 0, slab_hits: 0, slab_misses: 0 }
    }
    ...
    pub fn alloc(&mut self, size: usize) -> *mut u8 {
        if size <= 4096 && size >= 32 {
            // Tenta slab
            match self.slab.allocate(size) {
                Some(ptr) => { self.slab_hits += 1; return ptr; }
                None => { self.slab_misses += 1; }
            }
        }
