...
impl TtfRasterFont {
    ...
    pub fn load_from_bytes(&mut self, data: &[u8]) {
        ...
        let Some(count) = read_u32_le(data, 0).map(|v| v as usize) else {
            return;
        };
        ...
        for _ in 0..count {
            ...
            let Some(codepoint) = read_u32_le(data, offset) else {
                break;
            };
            let Some(w_raw) = read_u32_le(data, offset + 4) else {
                break;
            };
            let Some(h_raw) = read_u32_le(data, offset + 8) else {
                break;
            };
            let Some(x_raw) = read_i32_le(data, offset + 12) else {
                break;
            };
            let Some(y_raw) = read_i32_le(data, offset + 16) else {
                break;
            };
            ...
            offset += 20;
            let Some(pixels_size) = (w as usize).checked_mul(h as usize) else {
                break;
            };
            let Some(end) = offset.checked_add(pixels_size) else {
                break;
            };
            if end > data.len() {
                break;
            ...
            if let Some(glyph) = self.glyphs.get(&c) {
                        ...
                        if alpha > 0 {
                            ...
                            if rx < fb.info.width && ry < fb.info.height {
                                let blend = |bg: u8, fg: u8, a: u8| -> u8 {
                                    ((bg as u32 * (255 - a as u32) + fg as u32 * a as u32) / 255)
                                        as u8
                                };
                ...
                if let Some(bitmap) = crate::display::font::get_char_bitmap(c) {
                    for dy in 0..16 {
                        ...
                        for dx in 0..8 {
                            if (row >> (7 - dx)) & 1 == 1 {
                                fb.set_pixel(x + dx, y + dy + 4, r, g, b);
                            }
                        }
                    }
                    x += 10;
...
impl FontManager {
    ...
    pub fn activate(&mut self, name: &str) {
        if name == "vga" {
            self.use_ttf = false;
            self.active = String::from("vga");
        } else if self.fonts.contains_key(name) {
            self.use_ttf = true;
