...
| **AgentSpec manifest / Agency** | `agency.rs` + `native_agent_seed.rs` | Data-driven catalog: `AgentSpec` via `Agency::from_specs` (hermes wires). `agency_importer.rs` exists on disk but is **not** in `lib.rs` (orphan — do not claim wired). `native_agent_seed::load_all()` parses embedded SKILL.md. |
...
| `agency.rs` / `native_agent_seed.rs` | AgentSpec catalog, division grouping, embedded SKILL.md seed parsing. Orphans on disk (not in lib.rs): `agency_importer.rs`, `skill_snapshot.rs`, `workflow_learner.rs`, `fine_tuning_pipeline.rs` |
...
| `boot_observe.rs` | Observe DeviceTree → plano NIC+storage, cards PnP, Trust `(1,boot_observe,plan)`, HITL Escalate, HANR `hydrate_memory` |
| `economy.rs` / `context_window.rs` / `conversation.rs` / `shutdown.rs` / `hw_agents.rs` / `hw_capability.rs` / `boot_log_agent.rs` / `model_fit.rs` | Budget management (CompressionTier + record_tokens wired on InferQueue submit), LLM context windowing, curated conversation memory, orderly shutdown, HW-as-agent capabilities, HWID→family/fw/agent mapping, boot log persistence, fit-policy re-export. `skill_snapshot.rs` orphan (not in lib.rs). |
