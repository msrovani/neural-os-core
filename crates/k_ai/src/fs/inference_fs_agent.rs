...
fn generate_content(path: &str) -> Vec<u8> {
    ...
    alloc::format!(
        "[InferenceFS][synthetic/DEGRADED] {} @ tick {}\nTraining examples: {}\nGenerated on-demand (not model output)\n",
...
impl InferenceFsAgent {
    pub fn new() -> Self {
        k_nano::slog_kai!("INFERENCE", "ok", "/inference/ synthetic mount ready (DEGRADED)");
        InferenceFsAgent
    }
}
...
impl FilesystemAgent for InferenceFsAgent {
    ...
    fn write(&mut self, path: &str, data: &[u8]) -> Result<(), &str> {
        ...
        k_nano::slog_kai!("INFERENCE", "ok", "Training buffer: {} ({} bytes)", path, data.len());
        Ok(())
    }
