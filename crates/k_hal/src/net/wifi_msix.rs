...
/// Só sinaliza RX — EOI do LAPIC fica em `k_nano::apic` (camada de plataforma).
#[no_mangle]
pub unsafe extern "x86-interrupt" fn isr_wifi_rx(_frame: *mut u8) {
    RX_PENDING = true;
}

/// ISR de TX completo (sem EOI — ver `apic.rs`).
#[no_mangle]
...
impl AgnosticNetworkManager {
    ...
    pub unsafe fn poll(&mut self, hw_buf: &[u8]) {
        ...
        if let Some(pkt) = self.defrag.process(ctrl, hw_buf) {
            // FE hermes registra inject (evita ciclo k_hal→hermes)
            if let Some(f) = RX_INJECT {
                f(pkt);
            }
        }
    }
...
type RxInjectFn = fn(&[u8]);
static mut RX_INJECT: Option<RxInjectFn> = None;

/// hermes netstack registra isto no bring-up WiFi.
pub fn set_rx_inject(f: RxInjectFn) {
    unsafe { RX_INJECT = Some(f); }
}
