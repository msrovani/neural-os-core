...
// HardwareRegisterMap is defined in cortex crate (shared between cortex and hermes)
use cortex::HardwareRegisterMap;
...
impl WifiChipset for FallbackEthernet {
    fn init(&mut self) -> Result<(), &'static str> {
        // k-hal: eth FE vive em hermes; fallback marca Bound se NetPort Up/Bound
        let st = crate::net_port::status();
        match st {
            crate::net_port::NetPortStatus::Up | crate::net_port::NetPortStatus::Bound => {
                k_nano::slog_hal!("WIFI", "info", "Ethernet fallback (NetPort Bound/Up)");
                Ok(())
            }
            _ => {
                // PCI net class ainda conta como NIC presente
                let has = crate::discovery::device_tree()
                    .iter()
                    .any(|d| matches!(d.id.class, crate::device_cap::DeviceClass::Net));
                if has {
                    k_nano::slog_hal!("WIFI", "info", "Ethernet fallback (PCI net in DeviceTree)");
                    Ok(())
                } else {
                    Err("NIC ausente")
                }
...
pub unsafe fn runtime_probe_and_bind(vid: u16, did: u16, bar: usize)
    ...
    ACTIVE_DRIVER.lock(|active| {
        *active = None;
        ...
        k_nano::slog_hermes!("Wifi", "info", "{} detectado: {:04x}:{:04x} (bar={:#x}, ring={})", name, vid, did, bar, map.ring_size);
        Ok(())
    })
}
