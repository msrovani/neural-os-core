...
use k_nano::{serial_println, kjson};
...
impl GpuDevice {
    pub fn new(dev: &k_nano::pci::PciDevice, phys_mem_offset: u64) -> Option<Self> {
        serial_println!("[VGPU] BAR0={:#x} BAR1={:#x}", dev.bar0, dev.bar1);

        // Debug: dump todas as capabilities PCI
        unsafe {
            let caps_list = k_nano::pci::read_pci_capabilities(dev.bus, dev.device, dev.function);
            for (id, ptr) in &caps_list {
                if *id == 0x09 {
                    let cfg = k_nano::pci::read_config_byte(dev.bus, dev.device, dev.function, ptr + 3);
                    let bar = k_nano::pci::read_config_byte(dev.bus, dev.device, dev.function, ptr + 4);
                    let off = k_nano::pci::read_config_dword(dev.bus, dev.device, dev.function, ptr + 8);
                    let len = k_nano::pci::read_config_dword(dev.bus, dev.device, dev.function, ptr + 12);
        ...
        let (io_base, mmio_base, is_mmio) = unsafe {
            if let Some(cap) = k_nano::pci::read_virtio_cap(dev.bus, dev.device, dev.function, 1) {
                let bar_addr = k_nano::pci::read_bar_value(dev.bus, dev.device, dev.function, cap.bar);
                ...
                k_nano::apic::map_mmio_page(base, phys_mem_offset);
        ...
        if is_mmio && mmio_base > 0 {
            unsafe { k_nano::apic::map_mmio_page(mmio_base, phys_mem_offset); }
        ...
        unsafe {
            ...
            let notify_addr: u64 = if is_mmio {
                ...
                let notify_cap = k_nano::pci::read_virtio_cap(dev.bus, dev.device, dev.function, 2);
                if let Some(nc) = notify_cap {
                    let bar_val = k_nano::pci::read_bar_value(dev.bus, dev.device, dev.function, nc.bar);
            ...
            *avail.add(2) = 1;  // idx = 1 (updated AFTER ring entries)
            ...
            let poll_max = if k_nano::simd::has_whpx() { 20000 } else { 2000 };
...
unsafe fn init_framebuffer(io: &Regs, qpa: u64, cpa: u64, off: u64, width: u32, height: u32, notify_addr: u64) -> Option<GpuDevice> {
    ...
    *((cva as *mut u32).add(11)) = height;
    ...
    Some(GpuDevice { fb_addr: fb_pa, fb_width: width, fb_height: height, fb_stride: width*4, fb_bpp: 4, notify_addr, present: true, rgb_order: false })
}
