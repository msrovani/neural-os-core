...
pub fn run_benchmark() {
    ...
    k_nano::serial_println!("[BENCH] GPU Benchmark (TFLOPS) — matmul ternário");
    ...
    for &n in &sizes {
        ...
        let (a, b) = match (a_opt, b_opt) {
            (Some(a), Some(b)) => (a, b),
            _ => { k_nano::serial_println!("[BENCH] {}x{}: alloc failed", n, n); continue; }
        };

        let ops = 2.0 * (n as f64) * (n as f64) * (n as f64);

        let warm = gpu_matmul(&a, &b);
        if warm.is_none() { k_nano::serial_println!("[BENCH] {}x{}: matmul failed", n, n); continue; }
        ...
        k_nano::serial_println!("[BENCH] {}x{}: {:.3} TFLOPS ({:.1}s CPU, {} runs)", n, n, tflops, secs, runs);
    }
    k_nano::serial_println!("[BENCH] GPU backends: CPU fallback (v1.1.1 pipeline OK)");
}
