...
//! FFT audio: processa buffer AUDIO_IN para energia espectral.
...
pub fn process_audio_fft(pcm: &[i16]) {
    ...
    for bin in 0..16 {
        ...
        let mut chunk = pcm;
        // Processa em janelas de 32 samples
        while chunk.len() >= 32 {
            let mut s: f32 = 0.0;
            for i in 0..32 {
...
impl JarvisAvatar {
    ...
    pub fn render(&mut self, fb: &mut DoubleBuffer) {
        ...
        // Brilho central (legado — compositor já desenha o orb)
        let cx = self.fb_w as f32 / 2.0;
        let cy = self.fb_h as f32 / 2.0;
        self.draw_circle(fb, cx, cy, 4.0, 255, 255, 255, 60);
    }

    /// Só partículas + acentos de estado (orb base fica no compositor).
    pub fn render_particles(&mut self, fb: &mut DoubleBuffer) {
        ...
        match self.state {
            ...
            AvatarState::Listening => {
                ...
                self.draw_circle(fb, cx, cy, pulse, 0, 255, 255, 25);
