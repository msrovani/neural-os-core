...
/// Parâmetros para criação de uma partição no GPT.
#[derive(Debug, Clone, Copy)]
pub struct GptPartitionDef {
    pub type_guid: &'static [u8; 16],
    pub lba_start: u64,
    pub lba_end: u64,
    pub label: &'static str,
}

/// Cria GPT completa com N partições definidas por `parts`.
/// Retorna false se total_lba for pequeno demais ou write falhar.
pub fn gpt_format_multi(dev: &mut dyn BlockDevice, total_lba: u64, parts: &[GptPartitionDef]) -> bool {
    if total_lba <= 34 || parts.is_empty() { return false; }
    ...
    // Gera GUID do disco a partir dos labels
    let mut disk_guid = [0u8; 16];
    for part in parts {
        for (i, b) in part.label.bytes().enumerate() {
            disk_guid[i % 16] ^= b;
        }
    }
    ...
    for (i, part) in parts.iter().enumerate() {
        let off = i * 128;
        all_entries[off..off + 16].copy_from_slice(part.type_guid);
        all_entries[off + 16..off + 32].copy_from_slice(&disk_guid);
        all_entries[off + 32..off + 40].copy_from_slice(&part.lba_start.to_le_bytes());
        all_entries[off + 40..off + 48].copy_from_slice(&part.lba_end.to_le_bytes());
        // Label UTF-16LE
        let label_u16: Vec<u16> = part.label.encode_utf16().collect();
        for (j, &c) in label_u16.iter().enumerate().take(72 / 2) {
            all_entries[off + 56 + j * 2] = (c & 0xFF) as u8;
            all_entries[off + 57 + j * 2] = (c >> 8) as u8;
        }
    }
    ...
    hdr[40..48].copy_from_slice(&parts[0].lba_start.to_le_bytes()); // first usable
    // Safe fallback if parts somehow empty (defensive, parts guard at top of fn)
    let last_lba_val = parts.last().map(|p| p.lba_end).unwrap_or(last_lba);
    hdr[48..56].copy_from_slice(&last_lba_val.to_le_bytes()); // last usable
    ...
    // Limpa primeiro setor de cada partição
    let zero = [0u8; 512];
    for part in parts {
        dev.write_sectors(part.lba_start, &zero);
    }
    true
}
