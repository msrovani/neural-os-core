...
pub fn note_boot_msc_ok() {
    ...
    if tbl.count == 0 {
        ...
        crate::slog_bin!("USB-TRUST", "info", "seeded BOOT entry vid=0 pid=0");
    }
}

pub fn set_enforce(on: bool) {
    TABLE.lock().set_enforce(on);
    crate::slog_bin!("USB-TRUST", "info", "enforce={}", on);
}
...
pub fn load_bytes(data: &[u8]) -> Result<usize, &'static str> {
    ...
    crate::slog_bin!("USB-TRUST", "info", "loaded {} entries from usb.tbl", n);
    Ok(n)
}
...
pub fn enforce_deny_ports() {
    unsafe {
        ...
        crate::slog_bin!("USB-TRUST", "info", "disable_untrusted_ports n={}", n);
    }
}
