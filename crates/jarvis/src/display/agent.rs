...
use jarvis::display::fb::{DoubleBuffer, GPU};
use jarvis::display::compositor::{COMPOSITOR, JarvisDesktop, AppId, Layer, MOUSE_X, MOUSE_Y, MOUSE_BUTTONS};
use jarvis::display::avatar::JarvisAvatar;
...
impl Agent for DisplayAgent {
    ...
    fn tick(&mut self, tick: u64, _count: u64) -> AgentTickResult {
        ...
        if self.input_buffer.contains("+") {
            let v = jarvis::audio::settings::AUDIO_VOLUME.load(core::sync::atomic::Ordering::Relaxed);
            jarvis::audio::settings::AUDIO_VOLUME.store((v + 5).min(100), core::sync::atomic::Ordering::Relaxed);
        }
        if self.input_buffer.contains("-") {
            let v = jarvis::audio::settings::AUDIO_VOLUME.load(core::sync::atomic::Ordering::Relaxed);
            jarvis::audio::settings::AUDIO_VOLUME.store(v.saturating_sub(5), core::sync::atomic::Ordering::Relaxed);
        }
