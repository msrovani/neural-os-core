...
impl WifiChipset for AgnosticWifiEngine {
    fn init(&mut self) -> Result<(), &'static str> {
        ...
        k_nano::serial_println!("[WIFI-DMA] {} rings: TX@{:#x} RX@{:#x}", sz, tx_pa, rx_pa);
        Ok(())
    }
...
impl WifiChipset for FallbackEthernet {
    fn init(&mut self) -> Result<(), &'static str> {
        ...
        if has_nic {
            k_nano::serial_println!("[WIFI] Ethernet fallback (smoltcp ativo)");
            Ok(())
        } else {
            Err("NIC ausente")
        }
...
pub unsafe fn runtime_probe_and_bind(vid: u16, did: u16, bar: usize)
    ...
    ACTIVE_DRIVER.lock(|active| {
        *active = None;
        ...
        k_nano::serial_println!("[WIFI] {} detectado: {:04x}:{:04x} (bar={:#x}, ring={})",
...
pub fn detect_wifi() -> bool {
    ...
    k_nano::serial_println!("[WIFI] Nenhum adaptador wireless — Ethernet smoltcp ativo");
    false
}
