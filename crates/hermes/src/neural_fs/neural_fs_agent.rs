...
fn peek_config_txt(dev: &mut dyn BlockDevice) -> Option<Vec<u8>> {
    ...
    for p in &parts {
        ...
        let Some(mut ex) = k_nano::exfat::ExfatReader::new(dev, start) else {
            continue;
        };
...
impl NeuralFsAgent {
    ...
    fn try_exfat_write_smoke() {
        let mut guard = k_nano::ATA_DRIVER.lock();
        ...
        let parts = k_nano::fat32::read_mbr_dev(ata);
        for p in &parts {
            let start = p.lba_start as u64;
            let mut vbr = [0u8; 512];
            ...
            match k_nano::exfat_write::smoke_write_roundtrip(ata, start) {
    ...
    fn sync_usb_trust_table(&self) {
        // Enforce via CONFIG no ATA (mesmo peek do EXFAT_WRITE).
        {
            ...
            if let Some(ata) = guard.as_mut() {
                if config_flag_true(ata, "USB_TRUST_ENFORCE") {
                    k_nano::usb_trust::set_enforce(true);
                }
            }
        }
        let mut guard = self.state.lock();
        ...
        let loaded = Self::with_dev(st, |vol, dev| {
            ...
            let Some(ino) = vol.lookup_dir_entry(dev, trust, "usb.tbl") else {
                ...
                let blob = k_nano::usb_trust::serialize();
                if blob.len() > 12 {
                    let ino = vol.create_file(dev, trust, "usb.tbl")?;
                    vol.write_file(dev, ino, &blob)?;
                    k_nano::slog_bin!("USB-TRUST", "info", "created {}", k_nano::usb_trust::TBL_PATH);
                }
                return Ok(0usize);
            };
            let data = vol.read_file(dev, ino).map_err(|_| "read usb.tbl")?;
            k_nano::usb_trust::load_bytes(&data).map_err(|_| "parse usb.tbl")
        });
        match loaded {
        ...
        F: FnOnce(&mut NeuralVolume, &mut dyn BlockDevice) -> Result<R, &'static str>,
    {
        match &mut st.backend {
            ...
            Backend::Usb { .. } => {
                let mut guard = k_nano::globals::USB_MSC.lock();
    ...
    fn try_bootstrap_usb(&self) -> bool {
        ...
        if total < 16384 {
            k_nano::slog_bin!("NEURALFS", "info", "USB skip (sectors={} < 8MB)", total);
            return false;
        }
        let allow_fmt = usb_format_allowed(msc);
        if !allow_fmt {
            k_nano::slog_bin!("NEURALFS", "info", "USB format locked (set NEURALFS_USB_FORMAT=1 or debug build)");
        }
        ...
        for p in &parts {
            ...
            if NeuralVolume::probe_magic(dev, start) {
                if let Some(vol) = NeuralVolume::mount(dev, start) {
                    k_nano::slog_bin!("NEURALFS", "info", "{} mount LBA={} free_blocks={} inodes={}",
                        tag,
                        start,
                        vol.sb.free_blocks,
            ...
            if allow_format && total_lba >= 16384 {
                if NeuralVolume::format(dev, start, total_lba) {
                    if let Some(mut vol) = NeuralVolume::mount(dev, start) {
                        ...
                        k_nano::slog_bin!("NEURALFS", "info", "{} format+mount LBA={} size={}MB",
                            tag,
                            start,
    ...
    where
        ...
        F: FnOnce(u64, NeuralVolume) -> NeuralFsState,
    {
        let parts2 = k_nano::fat32::read_mbr_dev(dev);
        for p in &parts2 {
            if p.type_code != MBR_TYPE_NEURALFS {
                continue;
            }
            ...
            if let Some(mut vol) = NeuralVolume::mount(dev, start) {
                ...
                k_nano::slog_bin!("NEURALFS", "info", "{} mount after format LBA={}", tag, start);
                return Some(make(start, vol));
            }
        }
        None
    ...
    fn try_format_gpt_virgin<D: BlockDevice>(
        ...
        if !k_nano::gpt::gpt_format_single(
            dev,
            disk_sectors,
            &k_nano::gpt::GPT_TYPE_NEURALFS,
            "NeuralFS",
        ) {
            return false;
        }
        ...
        k_nano::slog_bin!("NEURALFS", "info", "{} GPT NeuralFS LBA={} size={}MB",
            tag,
            start,
            size * 512 / (1024 * 1024));
        true
    ...
    fn try_format_free_tail<D: BlockDevice>(
        ...
        let Some(off) = free_slot else {
            k_nano::slog_bin!("NEURALFS", "info", "{} no free MBR slot", tag);
            return false;
        };
        ...
        k_nano::slog_bin!("NEURALFS", "info", "{} created MBR 0x7F LBA={} size={}MB (dev/debug)",
            tag,
            start,
    ...
    fn bootstrap_ram(&self) {
        ...
        if !NeuralVolume::format(&mut disk, 0, total_lba) {
            k_nano::slog_bin!("NEURALFS", "info", "format RAM FAILED");
            return;
        }
        let Some(mut vol) = NeuralVolume::mount(&mut disk, 0) else {
            k_nano::slog_bin!("NEURALFS", "info", "mount RAM FAILED");
            return;
        };
        if let Ok(ino) = vol.create_file(&mut disk, 1, "hello.txt") {
            let _ = vol.write_file(&mut disk, ino, b"NeuralFS online\n");
        }
        k_nano::slog_bin!("NEURALFS", "info", "RAM 4MB mounted free_blocks={} inodes={}",
            vol.sb.free_blocks,
            vol.sb.allocated_inodes);
        *self.state.lock() = Some(NeuralFsState {
        ...
        if crate::neural_fs::tests::smoke_ram_roundtrip() {
            k_nano::slog_bin!("NEURALFS", "info", "smoke_ram_roundtrip=OK");
        } else {
            k_nano::slog_bin!("NEURALFS", "info", "smoke_ram_roundtrip=FAIL");
        }
        if crate::neural_fs::tests::smoke_reclaim() {
            k_nano::slog_bin!("NEURALFS", "info", "smoke_reclaim=OK");
        } else {
            k_nano::slog_bin!("NEURALFS", "info", "smoke_reclaim=FAIL");
        }
        if crate::neural_fs::tests::smoke_split() {
            k_nano::slog_bin!("NEURALFS", "info", "smoke_split=OK");
        } else {
