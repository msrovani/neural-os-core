...
//! Escada: L0 link → L1 MAC → L2 netstack → L3 static/DHCP → L3.5 RX prove → L4 DNS → L5 HTTP
...
pub fn bootstrap_early() {
    ...
    if s.bridge_mode {
        if crate::net::restore_dhcp_config() {
        ...
        if tick % 50 == 0 {
            log(
                ...
                &alloc::format!(
                    ...
                    crate::netstack::net_tx_count(),
                    crate::netstack::net_rx_count(),
                    k_nano::slip::slip_tx_count(),
                    k_nano::slip::slip_rx_count(),
                    NET_CONFIG.lock().configured
                ),
            );
        }
