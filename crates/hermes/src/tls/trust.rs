...
//! Canônico hermes::tls::trust (C5) — bin vira facade `pub use hermes_crate::tls::trust::*`.
...
use crate::tls::client::KernelRng;
...
pub fn load_pins_from_fat() {
    ...
    let Some(data) = cortex::gguf::read_fat_range(PIN_FILE, 0, 4096) else {
        k_nano::slog_hermes!("TLS", "info", "pins=FAT miss file={}", PIN_FILE);
        return;
    };
...
fn load_pins_from_sgdb() -> bool {
    ...
    match k_ai::sgdb::get_kv("sys/tls_pins") {
        Ok(Some(data)) => {
            let n = PINS.lock().load_bytes(&data);
            k_nano::slog_hermes!("TLS", "info", "pins=SGDB load n={} bytes={}", n, data.len());
            n > 0
        }
        Ok(None) => {
            k_nano::slog_hermes!("TLS", "info", "pins=SGDB miss");
            false
        }
        Err(e) => {
            k_nano::slog_hermes!("TLS", "info", "pins=SGDB load error={:?}", e);
            false
        }
    }
}
    ...
    fn verify_signature(&mut self, verify: CertificateVerifyRef) -> Result<(), TlsError> {
        ...
        if is_ecdsa {
            let Some(sec1) = self.leaf_sec1.take() else {
                set_certverify(CertVerifyResult::DenyNoLeaf);
                k_nano::slog_hermes!("TLS", "info", "certverify=DENY reason=no_leaf_sec1");
                return Err(TlsError::InvalidSignature);
            };
            ...
            return match EcdsaVerifier::verify(&vk, &msg[..msg_len], &sig) {
                ...
                Err(_) => {
                    set_certverify(CertVerifyResult::DenySig);
                    k_nano::slog_hermes!("TLS", "info", "certverify=DENY reason=ecdsa_bad_sig");
        ...
        let Some(der) = self.leaf_der.take() else {
            set_certverify(CertVerifyResult::DenyNoLeaf);
            k_nano::slog_hermes!("TLS", "info", "certverify=DENY reason=no_leaf_der_rsa");
            return Err(TlsError::InvalidSignature);
        };
        let Some((n, e)) = extract_rsa_ne(&der) else {
            set_certverify(CertVerifyResult::DenyNoLeaf);
            k_nano::slog_hermes!("TLS", "info", "certverify=DENY reason=no_rsa_spki");
            return Err(TlsError::InvalidSignature);
        };
        ...
        match RsaVerifier::verify(&vk, &msg[..msg_len], &sig) {
            Ok(()) => {
                ...
                k_nano::slog_hermes!("TLS", "info", "certverify=OK scheme=rsa_pss_sha256");
                Ok(())
            }
            Err(_) => {
                set_certverify(CertVerifyResult::DenySig);
                k_nano::slog_hermes!("TLS", "info", "certverify=DENY reason=rsa_pss_bad_sig");
                Err(TlsError::InvalidSignature)
            }
        }
    }
...
pub fn pin_ca_der(ca_der: &[u8]) -> bool {
    ...
    let fp = k_nano::tpm::sha256(ca_der);
...
pub fn issuer_pinned(issuer_der: &[u8]) -> bool {
    let fp = k_nano::tpm::sha256(issuer_der);
    let n = CA_N.load(Ordering::Relaxed) as usize;
