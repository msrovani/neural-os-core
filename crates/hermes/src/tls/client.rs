//! ADR-0016 N4 — cliente TLS 1.3 (`embedded-tls`) sobre NETSTACK (hermes).
//! Canônico hermes::tls::client (C5) — bin vira facade `pub use hermes_crate::tls::client::*`.
...
use crate::tls::trust::HybridProvider;
...
impl RngCore for KernelRng {
    fn next_u32(&mut self) -> u32 {
        k_nano::hw_rng::HardwareRandom::next_u64_retry(16).unwrap_or(0) as u32
    }

    fn next_u64(&mut self) -> u64 {
        k_nano::hw_rng::HardwareRandom::next_u64_retry(16).unwrap_or(0)
    }

    fn fill_bytes(&mut self, dest: &mut [u8]) {
        let _ = k_nano::hw_rng::HardwareRandom::fill_bytes(dest);
    }

    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), rand_core::Error> {
        k_nano::hw_rng::HardwareRandom::fill_bytes(dest)
...
pub struct NetTcpIo<'a> {
    stack: &'a mut crate::netstack::NetStack,
    handle: SocketHandle,
    now: u64,
}

impl<'a> NetTcpIo<'a> {
    pub fn new(stack: &'a mut crate::netstack::NetStack, handle: SocketHandle, now: u64) -> Self {
...
pub fn https_get_on_stack(
    ...
    if k_nano::env::is_sandbox() {
        k_nano::slog_hermes!("TLS", "info", "step=sandbox_skip (sem cert PKI)");
        return Err("tls_sandbox_skip");
    }
    k_nano::slog_hermes!("TLS", "info", "step=tcp_connect");
    let handle = stack
        .tcp_session_connect(ip, port, now)
        .ok_or("tls_tcp_connect")?;
    k_nano::slog_hermes!("TLS", "info", "step=tcp_ok");
    ...
    let result = (|| {
        ...
        k_nano::slog_hermes!("TLS", "info", "step=handshake");
        ...
        k_nano::slog_hermes!(
            ...
            crate::tls::trust::last_trust().as_str()
        );
        ...
        k_nano::slog_hermes!("TLS", "info", "step=http_sent");
