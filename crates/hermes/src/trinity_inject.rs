...
pub fn ensure_expert_resident(kind: ExpertKind) -> Option<usize> {
    ...
    let expert = router.get_or_mmap_expert(kind)?;
    if expert.weight.is_some() {
        Some(cortex::trinity::expert_resident_bytes(kind))
    } else {
        None
    }
}
...
mod tests {
    ...
    fn expert_names_match_registry() {
        ...
        // cada expert registrado tem o nome canônico do expert_kind_name
        for e in router.experts() {
            assert_eq!(cortex::trinity::expert_kind_name(e.kind), e.name);
        }
    }
