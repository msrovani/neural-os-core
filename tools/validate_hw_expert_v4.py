#!/usr/bin/env python3
"""validate_hw_expert_v4.py — validate the EXPORTED hw_expert_v4.bitnet artifact.

Standalone Python port of the Rust loader + predictor:
    crates/cortex/src/cortex.rs  load_hwexpert_v5 (lines ~244-351)
    crates/cortex/src/cortex.rs  predict_hw_v4    (lines ~383-520)
so it consumes the file EXACTLY as the kernel will (parse_end == file size,
packed ternary bits, embed column-major over (hidden,vocab), matmul layout
out[j] = sum_t W[t*n + j] * x[t], caps threshold logit > 0).

Checks performed:
  1. parse_end == file size; header fields (hidden=128 layers=6 heads=[17,8,9,10,9])
  2. nonzero fraction of backbone weights (q/k/v/o/g/u/d x 6 layers)  [GATE >= 1%]
  3. predictions for the 10 canonical test devices (must NOT be constant;
     well-known devices must map plausibly)
  4. holdout accuracy of the EXPORTED FILE (device-level, per head),
     split 90/10 by unique (vid,did), seed 42 (same as eval_hw_expert_v4.py)

Usage:
    python tools/validate_hw_expert_v4.py <path-to-bitnet>
"""
...
# ─── Vocab (mirror of models/hw_expert/v4/vocab.json) ────────────────────
FAMILY = [
...
def load_v5(data):
    ...
    # full re-parse from offset 14 (same as Rust)
    off = 14
    ...
    family_head, off = read_prefixed_ternary(data, off, hidden, 17)
...
def matmul_hybrid(w, x):
    """Rust scalar: out[j] = sum_t W[t*n + j] * x[t]; W flat (k=len(x), n=len(out))."""
    k = len(x)
    n = len(w) // k
    out = [0.0] * n
    for j in range(n):
...
def predict(model, vid, did):
    ...
    fam = argmax(matmul_hybrid(model["family_head"], pooled))
...
def predict_batch(model, vids, dids):
    """Batch of (vid, did) -> arrays fam/fw/ag/caps/nx. Same math as predict()."""
    h = model["hidden"]
    seq = 4
    v = model["vocab"]
    tokens = np.array([pack_vid_did(a, b, v) for a, b in zip(vids, dids)], dtype=np.int64)
    emb = _mflat(model["embed"], v, h)          # (vocab, h) — F[col*h+row]
    H = emb[tokens]                              # (B, seq, h) float32
    for layer in model["layers"]:
        ...
        # attention: v then o (only q_dim outputs used)
        vv = H @ Mv
        oo = vv @ Mo
        qd = layer["q_dim"]
        H = H.copy()
    ...
    pooled = H.mean(axis=1)                       # (B, h)
    Mfam = _mflat(model["family_head"], h, 17)
...
# ─── Holdout split (mirrors tools/eval_hw_expert_v4.py, seed 42) ─────────
def load_samples():
...
def validate(path, quiet=False):
    ...
    out["heads"] = head_cols
    out["header_ok"] = (m["hidden"] == 128 and m["num_layers"] == 6
                        and head_cols == [17, 8, 9, 10, 9])
    out["nz_frac"] = nonzero_fraction(m)
    out["nz_gate"] = out["nz_frac"] >= 0.01

    # predictions for canonical test devices (exact scalar port)
    rows = []
    fams = set()
    ...
    out["test_gate"] = len(fams) >= 2
    out["vec_consistent"] = vec_consistent

    # holdout accuracy of the exported file (device-level, per head)
    ...
    out["family_gate"] = out["holdout_acc"]["family"] >= 60.0
    ...
    if not quiet:
        ...
        print(f"  family GATE >= 60%: {out['family_gate']}")
    return out
