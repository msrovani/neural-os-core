...
$args = @("-m","6G","-smp","2","-accel","tcg","-drive","format=raw,file=$uefi,if=ide,index=0") + $diskArg + @("-drive","if=pflash,format=raw,file=$ovmf,readonly=on","-serial","file:$logfile","-serial","tcp:127.0.0.1:4444","-nic","user,model=e1000,hostfwd=tcp::4445-:4445,hostfwd=tcp::4446-:4446")
