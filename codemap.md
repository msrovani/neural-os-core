...
AI-native bare-metal OS written in Rust (`no_std` + `no_main`, x86_64). Everything is an Agent or a Skill — no tasks, no services, no standalone drivers. Hardware rings: Ring 0 (NPU — intent routing), Ring 1 (GPU — tensor), Ring 2 (CPU — agents/skills). ~26.000 LOC, 180+ files, ~50 native agents. Boot via Limine UEFI (bootloader 0.11.15), 8-phase event-driven sequence (SafeHarbor → MemoryCore → SystemBringup → Diagnostics → HardwareDiscovery → DriverInit → AgentFleet → Runtime). Current line: v1.9.9 TEST, K³CHJ = k-nano + k-hal + k-ai + Cortex + Hermes + Jarbas.
...
|-----------|------------------------|--------------|
| `crates/k_nano/` | Ring 0 hardware foundation — the crate every product crate depends on. Owns CPU/platform bring-up (bitmap frame allocator, 512MB heap, IDT/GDT/TSS/APIC/ACPI, PCI, SMP directed wake), all device drivers (NICs with UC-mapped DMA rings, xHCI, storage), filesystems + VFS, scheduler/sync primitives, async runtime, MHI tiering, P2P mesh transport, and the canonical cross-crate singletons (GLOBAL_ALLOCATOR, PHYS_MEM_OFFSET, EVENT_BUS, SKILL_REGISTRY, nic_globals, smp::ap_entry). | [View Map](crates/k_nano/codemap.md) |
